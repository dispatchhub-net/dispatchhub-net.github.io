        ...
        <div class="bg-gray-900 border border-white/10 rounded-3xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden relative">
            ...
            <div class="relative z-10 flex flex-col md:flex-row justify-between items-center px-8 py-6 border-b border-white/5 backdrop-blur-sm bg-gray-900/50">
                ...
                <div class="flex items-center gap-6">
                    <div class="flex bg-black/40 p-1 rounded-xl ring-1 ring-white/10 backdrop-blur-md overflow-x-auto">
                        ...
                        ${(appState.loads?.contractTypes || []).map(c => `
                            ...
                            <button onclick="window.setRecordsFilter('${c.id}')" class="px-6 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === c.id ? 'bg-gray-700 text-white shadow-lg ring-1 ring-white/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}">${c.id}</button>
                        `).join('')}
                    </div>
