...
export const updateHallOfFameFromSession = async () => {
    ...
    stubs.forEach(stub => {
        const type = (stub.contract_type || stub.contract || 'UNKNOWN').toUpperCase();
        ...
        if (!weeklyAggregates[aggKey]) weeklyAggregates[aggKey] = {};
        if (!weeklyAggregates[aggKey][type]) weeklyAggregates[aggKey][type] = createAgg();
    ...
    loads.forEach(load => {
        ...
        const type = (load.contract_type || load.contract || 'UNKNOWN').toUpperCase();
