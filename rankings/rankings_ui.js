...
const getCompositeTeamName = (load) => {
    ...
    const specialPrefixes = ['agnius', 'miles', 'uros', 'alex'];
...
function captureCurrentViewState() {
    ...
    visibleColumns: Array.from(appState.visibleColumns[appState.rankingMode].entries()),
...
function applyViewState(viewState) {
    ...
    appState.visibleColumns[appState.rankingMode] = new Map(viewState.visibleColumns);
...
export function loadDefaultView() {
    const visibleColumnsMapDispatcher = new Map();
    const visibleColumnsMapTeam = new Map();
    allColumns.forEach(col => {
        visibleColumnsMapDispatcher.set(col.id, col.defaultVisible);
        visibleColumnsMapTeam.set(col.id, col.defaultVisible);
    });
    
    appState.rankingMode = 'dispatcher';
    appState.filters = [];
    appState.minDriverSetting = { type: 'numDrivers', value: 3 };
    appState.visibleColumns.dispatcher = visibleColumnsMapDispatcher;
    appState.visibleColumns.team = visibleColumnsMapTeam;
...
export function load1WkAllView() {
    ...
    const oneWkColumnIds = new Set([
        ...
        'pMainCriteriaNetDriverMargin_current', 'pMainCriteria2CashFlow_current', 'rpmAll_current', 'mainCriteria_current'
    ]);
    ...
    appState.visibleColumns[appState.rankingMode] = visibleColumnsMap;
...
export function load4WkAllView() {
    ...
    const fourWkColumnIds = new Set([
        ...
        'pMainCriteriaNetDriverMargin_4wkAvg', 'pMainCriteria2CashFlow_4wkAvg', 'rpmAll_4wkAvg', 'mainCriteria_4wkAvg'
    ]);
    allColumns.forEach(col => {
...
export const renderUI = () => {
    ...
    if (!appState.loading && !appState.error) {
        ...
        const loader = document.getElementById('rankings-global-loader');
...
const renderKeyMetrics = () => {
    ...
    const getCardData = (metricId) => {
        ...
        if (baseId === 'compliance') metricInfo.label = 'Compliance';
        ...
        const iconMap = {
            ...
            compliance: `<div class="p-0.5 rounded-full bg-cyan-600 bg-opacity-20 text-cyan-400"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></div>`,
        };
...
const applyStickyStyles = () => {
    ...
    const visibleCols = orderedColumns.filter(col => {
        ...
        return appState.visibleColumns[appState.rankingMode].get(col.id);
    });
...
export const renderMainTable = () => {
    ...
    if (!allColumns.find(c => c.id === 'company')) {
            const teamColIndex = allColumns.findIndex(c => c.id === 'dispatcherTeam');
            const insertIndex = teamColIndex >= 0 ? teamColIndex + 1 : allColumns.length;
            allColumns.splice(insertIndex, 0, { 
                id: 'company', 
                label: 'Company', 
                type: 'string', 
                width: '', 
        ...
        if (!appState.visibleColumns.dispatcher.has('company')) appState.visibleColumns.dispatcher.set('company', true);
        if (!appState.visibleColumns.team.has('company')) appState.visibleColumns.team.set('company', false);

        if (!allColumns.find(c => c.id === 'compliance_current')) {
            let targetIndex = allColumns.findIndex(c => c.id === 'pMainCriteria2CashFlow_4wkAvg');
            if (targetIndex === -1) {
                targetIndex = allColumns.findIndex(c => c.id === 'pMainCriteria2CashFlow_current');
            }
            const insertIndex = targetIndex >= 0 ? targetIndex + 1 : allColumns.length;
            allColumns.splice(insertIndex, 0, { 
                id: 'compliance_current', 
                label: 'Compliance', 
                type: 'number', 
                unit: '%',
                defaultVisible: true,
        ...
        if (!appState.visibleColumns.dispatcher.has('compliance_current')) appState.visibleColumns.dispatcher.set('compliance_current', true);
        if (!appState.visibleColumns.team.has('compliance_current')) appState.visibleColumns.team.set('compliance_current', true);

        if (!allColumns.find(c => c.id === 'compliance_4wkAvg')) {
            const targetIndex = allColumns.findIndex(c => c.id === 'compliance_current');
            const insertIndex = targetIndex >= 0 ? targetIndex + 1 : allColumns.length;
            allColumns.splice(insertIndex, 0, { 
                id: 'compliance_4wkAvg', 
                label: 'Compliance (4W)', 
                type: 'number', 
                unit: '%',
                defaultVisible: true,
        ...
        if (!appState.visibleColumns.dispatcher.has('compliance_4wkAvg')) appState.visibleColumns.dispatcher.set('compliance_4wkAvg', true);
        if (!appState.visibleColumns.team.has('compliance_4wkAvg')) appState.visibleColumns.team.set('compliance_4wkAvg', true);
    ...
    const sortedData = [...filteredData].sort((a, b) => {
        ...
        let bValue = b[key];
        if (key === 'company') {
            const getCompanyStr = (ent) => {
                if (appState.rankingMode === 'team' && ent.companyNames) {
                    return ent.companyNames.length > 0 ? ent.companyNames.join(', ').toLowerCase() : '-';
                }
                let cStr = ent.company || ent.company_name;
                if (cStr && cStr !== '-' && cStr.trim() !== '') {
                    return typeof cStr === 'string' ? cStr.replace(/ - /g, ', ').toLowerCase() : String(cStr).toLowerCase();
                }
                return '-';
            };
            aValue = getCompanyStr(a);
            bValue = getCompanyStr(b);
        }
        if (key === 'rank') {
            aValue = a['_sortRank'];
            bValue = b['_sortRank'];
        ...
        if (typeof aValue === 'string' && typeof bValue === 'string') {
            let valA = aValue === '-' ? '\uFFFF' : aValue;
            let valB = bValue === '-' ? '\uFFFF' : bValue;
            const comparison = valA.localeCompare(valB);
            return appState.sortConfig.direction === 'ascending' ? comparison : -comparison;
        }
    ...
    const sortColumn = allColumns.find(c => c.id === appState.sortConfig.key);
    let isReverse = false;
    if (sortColumn) {
        if (sortColumn.type === 'string' || sortColumn.id === 'rank') {
            isReverse = appState.sortConfig.direction === 'descending';
        } else {
            isReverse = appState.sortConfig.direction === 'ascending';
        }
    }
    const totalEligible = eligibleForRanking.length;
    const finalVisibleRanks = new Map(
        eligibleForRanking.map((d, i) => [
            d.entityName, 
            isReverse ? totalEligible - i : i + 1
        ])
    );
            ...
            if (shouldRedact && columnsToRedact.has(baseMetricId)) {
                ...
                switch (col.id) {
                    case 'rank':
                        ...
                        break;
                    case 'company':
                        if (appState.rankingMode === 'team' && entity.companyNames) {
                            if (entity.companyNames.length === 0) {
                                content = `<span class="text-gray-300 font-medium">-</span>`;
                            } else if (entity.companyNames.length === 1) {
                                content = `<span class="text-gray-300 font-medium">${entity.companyNames[0]}</span>`;
                            } else {
                                const tooltipText = entity.companyNames.join(', ');
                                content = `<span class="text-gray-300 font-medium cursor-help group relative">
                                    Multiple
                                    <span class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block bg-gray-700 text-gray-100 text-xs rounded-md py-1 px-2 whitespace-nowrap z-50 shadow-lg">
                                        ${tooltipText}
                                    </span>
                                </span>`;
                            }
                        } else {
                            let companyStr = entity.company || entity.company_name;
                            if (companyStr && companyStr !== '-' && companyStr.trim() !== '') {
                                 if (typeof companyStr === 'string') {
                                    companyStr = companyStr.replace(/ - /g, ', ');
                                }
                            } else {
                                companyStr = '-';
                            }
                            content = `<span class="text-gray-300 font-medium">${companyStr}</span>`;
                        }
                        break;
                    case 'entityName':
                        ...
                        const safeName = typeof value === 'string' ? value.replace(/'/g, "\\'") : value;
                        const cleanName = typeof value === 'string' ? value.trim().toLowerCase() : '';
                        
                        // Force calculate stats if they are missing
                        if (window.DispatchersUI && (!window.DispatchersUI.state.stats || Object.keys(window.DispatchersUI.state.stats).length === 0)) {
                            try { window.DispatchersUI.calculateGlobalRankings(); window.DispatchersUI.processStats(); } catch(e) {}
                        }
                        
                        const isInPerformance = window.DispatchersUI?.validPerformanceNames?.has(cleanName);

                        const profileIcon = (isAdmin() && appState.rankingMode === 'dispatcher' && !entity._isBelowMinDrivers && isInPerformance) ? 
                            `<div onclick="event.stopPropagation(); window.openDispatcherProfile('${safeName}')" class="ml-2 cursor-pointer text-gray-500 hover:text-teal-400 transition-colors opacity-0 group-hover:opacity-100" title="View Profile">${SILHOUETTE_ICON_SVG}</div>` : '';

                        content = `<div class="flex items-center group">${statusCircle}<span class="text-gray-200">${value === null ? '-' : value}</span>${profileIcon}${entityTrophy}</div>`;
                        break;
...
export const renderColumnVisibilityDropdown = () => {
    ...
    allColumns.forEach(col => {
        if (appState.rankingMode === 'team' && (col.id === 'dispatcherTeam' || col.id === 'weeksIncluded_4wkAvg')) return;
        if (appState.rankingMode === 'dispatcher' && col.id === 'numDispatchers') return;

        const isVisible = appState.visibleColumns[appState.rankingMode].get(col.id);
        const isPinnedLeft = appState.pinnedLeftColumns.includes(col.id);
        const isPinnedRight = appState.pinnedRightColumns.includes(col.id);
...
const renderBumpChartSection = () => {
    ...
    rankByOptions.push({ value: 'compliance', label: 'Compliance' });
    rankByOptions.push({ value: 'compliance_4wkAvg', label: 'Compliance (4W)' });
...
const populateCriteriaWeightsSettings = () => {
    if (!isAdmin()) {
        let container = document.getElementById('criteria-weights-settings-container');
        if (container) {
    ...
    const modalTitle = document.querySelector('#display-settings-modal h2');
    if (modalTitle) {
        modalTitle.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 mr-2"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0-.33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1.82.33h.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33-1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-.33 1.82Z"></path></svg> Settings`;
    }

    let container = document.getElementById('criteria-weights-settings-container');
    const modalBody = document.querySelector('#display-settings-modal .p-4.space-y-4');

    if (!container && modalBody) {
        container = document.createElement('div');
        container.id = 'criteria-weights-settings-container';
        modalBody.insertBefore(container, modalBody.firstChild);
        
        const separator = document.createElement('hr');
        separator.className = 'border-gray-700 my-4';
        modalBody.insertBefore(separator, container.nextSibling);
        
        const displaySettingsHeader = document.createElement('h3');
        displaySettingsHeader.className = 'text-base font-bold text-gray-200 mt-4 mb-2';
        displaySettingsHeader.textContent = 'Display Settings';
        modalBody.insertBefore(displaySettingsHeader, separator.nextSibling);
    }
    
    if (!container) return;

    const wD = appState.criteriaWeights?.driverHappiness ?? 50;
    const wC = appState.criteriaWeights?.companyHappiness ?? 50;
    const wComp = appState.criteriaWeights?.compliance ?? 0;

    container.innerHTML = `
        <h3 class="text-base font-bold text-gray-200 mb-3">Criteria Calculation</h3>
        <div class="space-y-3">
            <div class="flex items-center justify-between">
                <label class="text-sm text-gray-300">Driver Happiness Weight (%)</label>
                <input type="number" id="weight-driver" class="w-16 bg-gray-900 border border-gray-600 rounded px-2 py-1 text-white text-sm text-center" value="${wD}">
            </div>
            <div class="flex items-center justify-between">
                <label class="text-sm text-gray-300">Company Happiness Weight (%)</label>
                <input type="number" id="weight-company" class="w-16 bg-gray-900 border border-gray-600 rounded px-2 py-1 text-white text-sm text-center" value="${wC}">
            </div>
            <div class="flex items-center justify-between">
                <label class="text-sm text-gray-300">Compliance Weight (%)</label>
                <input type="number" id="weight-compliance" class="w-16 bg-gray-900 border border-gray-600 rounded px-2 py-1 text-white text-sm text-center" value="${wComp}">
            </div>
            <div class="flex justify-end pt-2">
                <button id="save-criteria-weights" class="bg-teal-600 hover:bg-teal-500 text-white text-sm font-semibold px-4 py-1.5 rounded transition-colors">Save Weights</button>
            </div>
        </div>
    `;

    document.getElementById('save-criteria-weights').addEventListener('click', async () => {
        const dVal = parseFloat(document.getElementById('weight-driver').value) || 0;
        const cVal = parseFloat(document.getElementById('weight-company').value) || 0;
        const compVal = parseFloat(document.getElementById('weight-compliance').value) || 0;
        
        if (dVal + cVal + compVal !== 100) {
            alert('The sum of percentages must be exactly 100!');
            return;
        }
        
        const btn = document.getElementById('save-criteria-weights');
        btn.textContent = 'Saving...';
        btn.disabled = true;
        
        import('./rankings_api.js').then(async (api) => {
            if (api.saveCriteriaWeights) {
                await api.saveCriteriaWeights({ driverHappiness: dVal, companyHappiness: cVal, compliance: compVal });
            }
            if (api.applyWeightsToHistoricalData) api.applyWeightsToHistoricalData();
            api.getOrComputeHistoricalMetrics();
            api.processDataForMode();
            renderUI();
            btn.textContent = 'Saved!';
            btn.disabled = false;
            setTimeout(() => {
                if (btn) btn.textContent = 'Save Weights';
            }, 2000);
        });
    });
};
...
const renderStubsTable = () => {
    ...
    if (appState.rankingMode === 'dispatcher') {
        ...
        // --- NEW: Calculate Tenure for each stub ---
        const allHistoricalStubs = appState.loads?.historicalStubsData || [];
        const targetDateStr = appState.selectedDate; // Use YYYY-MM-DD string to ignore time matching issues
        
        allStubs = allStubs.map(stub => {
            const dName = stub.driverName;
            const driverHistory = allHistoricalStubs
                .filter(s => {
                    if (!s.pay_date) return false;
                    const sDate = new Date(s.pay_date).toISOString().split('T')[0];
                    return s.driver_name === dName && sDate <= targetDateStr;
                })
            ...
            for (const s of driverHistory) {
                const miles = parseFloat(s.total_miles || 0);
                const gross = parseFloat(s.driver_gross || 0);
                if (!startedCounting && (miles > 0 || gross > 0)) {
            ...
            const tenureDisplay = count >= 10 ? '10+ wks' : (count > 0 ? count + ' wks' : '-');
            return { ...stub, tenure: tenureDisplay, _tenureRaw: count };
        });
        // -------------------------------------------

        const headers = [
            { label: 'Driver Name', key: 'driverName', type: 'string' },
            { label: 'Tenure', key: 'tenure', type: 'number' },
            { label: 'Contract Type', key: 'contractType', type: 'string' },
        ...
        allStubs.sort((a, b) => {
            ...
            if (sortKey === 'criteria') {
                 ...
                 aValue = a._tenureRaw || 0;
                 bValue = b._tenureRaw || 0;
            }
...
const renderDispatcherDriverDetails = (dispatcher) => {
    ...
    // --- NEW: Calculate Tenure for each stub (Team Mode) ---
    const allHistoricalStubs = appState.loads?.historicalStubsData || [];
    const targetDateStr = dispatcher.date ? new Date(dispatcher.date).toISOString().split('T')[0] : appState.selectedDate;
    
    filteredDriverStubs = filteredDriverStubs.map(stub => {
        const dName = stub.driverName;
        const driverHistory = allHistoricalStubs
            .filter(s => {
                if (!s.pay_date) return false;
...
const populateDisplaySettingsModal = () => {
    ...
    const modalBody = document.querySelector('#display-settings-modal .p-4.space-y-4') || document.querySelector('#display-settings-modal .p-6.space-y-6');
    let container = document.getElementById('criteria-weights-settings-container');
    
    if (!container && modalBody) {
        modalBody.className = 'p-6 space-y-6 text-sm overflow-y-auto max-h-[80vh]';

        container = document.createElement('div');
        container.id = 'criteria-weights-settings-container';
        container.className = 'bg-gray-800 p-5 rounded-xl border border-gray-600 shadow-inner';
        modalBody.insertBefore(container, modalBody.firstChild);
        
        const displaySettingsContainer = document.createElement('div');
        displaySettingsContainer.className = 'bg-gray-800 p-5 rounded-xl border border-gray-600 shadow-inner space-y-4';
        
        const displaySettingsHeader = document.createElement('h3');
        displaySettingsHeader.className = 'text-lg font-bold text-gray-200 mb-4 border-b border-gray-700 pb-2';
        displaySettingsHeader.textContent = 'Display Settings';
        displaySettingsContainer.appendChild(displaySettingsHeader);
        
        const existingContentNodes = Array.from(modalBody.children).filter(node => node.id !== 'criteria-weights-settings-container' && node !== displaySettingsContainer);
        existingContentNodes.forEach(node => displaySettingsContainer.appendChild(node));
        
        modalBody.appendChild(displaySettingsContainer);
        
        const separators = modalBody.querySelectorAll('hr');
    ...
    if (container) {
        if (!isAdmin()) {
            ...
            container.innerHTML = `
                <h3 class="text-lg font-bold text-gray-200 mb-4 border-b border-gray-700 pb-2">Criteria Calculation</h3>
                <div class="space-y-4 mb-2">
                    <div class="flex items-center justify-between">
                        <label class="text-sm font-medium text-gray-300">Driver Happiness Weight (%)</label>
                        <input type="number" id="weight-driver" class="w-20 bg-gray-900 border border-gray-600 rounded px-2 py-1 text-white text-sm text-center focus:ring-teal-500 focus:border-transparent" value="${wD}">
                    </div>
                    <div class="flex items-center justify-between">
                        <label class="text-sm font-medium text-gray-300">Company Happiness Weight (%)</label>
                        <input type="number" id="weight-company" class="w-20 bg-gray-900 border border-gray-600 rounded px-2 py-1 text-white text-sm text-center focus:ring-teal-500 focus:border-transparent" value="${wC}">
                    </div>
                    <div class="flex items-center justify-between mb-2">
                        <label class="text-sm font-medium text-gray-300">Compliance Weight (%)</label>
                        <input type="number" id="weight-compliance" class="w-20 bg-gray-900 border border-gray-600 rounded px-2 py-1 text-white text-sm text-center focus:ring-teal-500 focus:border-transparent" value="${wComp}">
                    </div>
                    <div class="flex justify-end pt-4 mt-2 border-t border-gray-700">
                        <button id="save-criteria-weights" class="bg-teal-600 hover:bg-teal-500 text-white text-sm font-semibold px-4 py-2 rounded transition-colors shadow-lg">Save Weights</button>
                    </div>
                </div>
            `;

            const saveBtn = document.getElementById('save-criteria-weights');
            ...
            return;
                    ...
                    import('./rankings_api.js').then(async (api) => {
                        await api.saveCriteriaWeights({ driverHappiness: dVal, companyHappiness: cVal, compliance: compVal });
                        api.applyWeightsToHistoricalData();
                        api.getOrComputeHistoricalMetrics();
                        api.processDataForMode();
                        renderUI();
                        if (appState.isEntityModalOpen) {
                            import('./rankings_ui.js').then(ui => ui.renderModalContent());
                        }
                        saveBtn.textContent = 'Saved!';
                        saveBtn.disabled = false;
                        setTimeout(() => {
                            if (saveBtn) saveBtn.textContent = 'Save Weights';
                        }, 2000);
                    });
                });
...
const populateKeyMetricsSettings = () => {
    ...
    const selectableMetrics = [
        ...
        { id: 'compliance_4wkAvg', label: 'Compliance (4W)' },
    ...
    const dropdownOptions = document.createElement('div');
    dropdownOptions.className = "absolute bottom-full left-0 right-0 mb-1 bg-gray-700 border border-gray-600 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto hidden";
...
export const renderIndividualEntityChart = () => {
    ...
    const xAxis = d3.axisBottom(xScale).tickFormat(d3.utcFormat("%b %d"));
    ...
    svg.append("rect").attr("width", width).attr("height", height).style("fill", "none").style("pointer-events", "all")
        ...
        .on("mousemove", function(event) {
            ...
            let tooltipHtml = `<strong class="font-bold">${d3.utcFormat("%Y-%m-%d")(primaryDataPoint.date)}</strong><br/>`;
...
export const renderD3BumpChart = (bumpChartData, selectedEntityNames) => {
    ...
    const xAxis = d3.axisBottom(xScale).ticks(d3.utcWeek.every(1)).tickFormat(d3.utcFormat("%b %d")).tickSizeOuter(0);
    ...
    svg.append("rect").attr("width", width).attr("height", height).style("fill", "none").style("pointer-events", "all")
        ...
        .on("mousemove", function(event) {
            ...
            let tooltipHtml = `<strong>Date:</strong> ${d3.utcFormat("%Y-%m-%d")(d.date)}<br/>`;
            selectedEntityNames.forEach(name => {
                const entityData = d[name];
                if (entityData) {
                    let formattedValue = '';
                    if (appState.bumpMetric.includes('compliance')) {
                        formattedValue = `(${entityData.value.toFixed(1)}%)`;
                    } else if (metricInfo && metricInfo.unit === '%') {
                        formattedValue = `(${(entityData.value * 100).toFixed(1)}%)`;
                    } else if (metricInfo && metricInfo.unit === '$') {
                        formattedValue = `($${entityData.value.toFixed(2)})`;
                    } else if (typeof entityData.value === 'number') {
                        formattedValue = `(${entityData.value.toFixed(0)})`;
                    }
...
export function toggleColumnVisibility(columnId) {
    const newMap = new Map(appState.visibleColumns[appState.rankingMode]);
    newMap.set(columnId, !newMap.get(columnId));
    appState.visibleColumns[appState.rankingMode] = newMap;
    renderUI(); 
}
...
export const updateDriverTypeSwitcherUI = () => {
    const switcherContainer = document.getElementById('driver-type-switcher');
    
    // Generate buttons dynamically if empty
    if (switcherContainer && switcherContainer.children.length === 0) {
        const types = appState.loads?.contractTypes || [{id: 'OO'}, {id: 'LOO'}];
        let buttonsHtml = `<button data-type="all" class="px-3 py-1 text-xs font-medium rounded-md transition-colors duration-200 h-full">ALL</button>`;
        types.forEach(c => {
            buttonsHtml += `<button data-type="${c.id.toLowerCase()}" class="px-3 py-1 text-xs font-medium rounded-md transition-colors duration-200 h-full">${c.id}</button>`;
        });
        switcherContainer.innerHTML = buttonsHtml;
    }
                    ...
                    option.onclick = () => {
                        ...
                        if (entityRawData.length > 0) {
                            if (isTeamMode) {
                                ...
                                const consolidated = {
                                    dispatcherName: entityRawData[0].dispatcherName,
                                    company: entityRawData[0].company || entityRawData[0].companyName || entityRawData[0].company_name || '',
                                    dispatcherTeam: [...new Set(entityRawData.map(r => r.dispatcherTeam))].join(', '),
                                    date: entityRawData[0].date,
                                ...
                                const weightedCompliance = entityRawData.reduce((sum, r) => sum + ((r.compliance || 0) * (r.numDrivers || 0)), 0);
                                consolidated.compliance = totalWeight > 0 ? weightedCompliance / totalWeight : 0;
...
function renderDispatcherHeatmap() {
    ...
    const getFilteredDataForEntity = (entityName) => {
        ...
        if (driverTypeFilter !== 'all') {
            baseData = baseData.filter(load => {
                const contractType = (load.contract_type || 'UNKNOWN').trim().toUpperCase();
                return contractType === driverTypeFilter.toUpperCase();
            });
        }
