...
function renderDispatcherHeatmap() {
    ...
    if (isDriverView && appState.entityModalHeatmapDriver) {
        ...
        if (renderArea) renderDriverRouteMap(renderArea, driverLoads, directionView, tooltip);
            ...
            <div class="relative flex-grow min-h-0">
                <div class="absolute inset-0 overflow-y-auto rounded-lg">
                    <table class="w-full text-xs text-left heatmap-loads-table">
                        <thead class="text-xs text-gray-400 uppercase sticky top-0 bg-gray-900/70 backdrop-blur-sm z-10">
                            <tr>
                                ${isTeamView ? '<th style="width: 20%;">Dispatcher</th>' : ''}
                                <th style="width: ${isTeamView ? '10%' : '12%'}">PU Date</th>
                                <th style="width: ${isTeamView ? '20%' : '28%'}">PU</th>
                                <th style="width: ${isTeamView ? '20%' : '28%'}">DEL</th>
                                <th style="width: ${isTeamView ? '10%' : '12%'}">DEL Date</th>
                                <th style="width: 10%;" class="text-right">Rate</th>
                                <th style="width: 10%;" class="text-right">RPM</th>
                            </tr>
                        </thead>
                        ...
                        ${sortedLoads.map(load => {
                            let colorClass = 'text-white'; // Default for non-comparison
                            if (isComparing) {
                                colorClass = load.__color === '#86efac' ? 'text-green-400-pale' : 'text-red-400-pale';
                            }
                            const formatDate = (dateString) => dateString ? new Date(dateString).toLocaleDateString('en-US', {month: '2-digit', day: '2-digit'}) : '-';
                            const compositeTeamName = getCompositeTeamName(load);
                            const canView = appState.rankingMode === 'team' ? canViewTeam(compositeTeamName) : canViewDispatcher(load.dispatcher);
                            const rateContent = canView && !isDispatcherRoleInTeamMode ? `$${(load.price || 0).toLocaleString()}` : '-';
                            const rpmContent = canView && !isDispatcherRoleInTeamMode ? `$${(load.rpm_all || 0).toFixed(2)}` : '-';
                            const dispatcherCell = isTeamView ? `<td class="${colorClass}">${load.dispatcher || '-'}</td>` : '';

                            return `
                                <tr>
                                    ${dispatcherCell}
                                    <td class="${colorClass}">${formatDate(load.pu_date)}</td>
                                    <td class="${colorClass}">${load.pu_location || '-'}</td>
                                    <td class="${colorClass}">${load.do_location || '-'}</td>
                                    <td class="${colorClass}">${formatDate(load.do_date)}</td>
                                    <td class="text-right font-mono ${colorClass}">${rateContent}</td>
                                    <td class="text-right font-mono ${colorClass}">${rpmContent}</td>
                                </tr>`;
                        }).join('')}
                        </tbody>
                    </table>
