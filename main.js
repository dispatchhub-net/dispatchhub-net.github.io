...
import { HISTORICAL_STUBS_URLS, DRIVER_COUNT_LIVE_URL, CONTRACT_STATUS_URL, SETTINGS_APPS_SCRIPT_URL } from './config.js'; // Import SETTINGS_APPS_SCRIPT_URL
...
const addEventListeners = () => {
    ...
    const switchView = async (view) => {
        ...
        if (view === 'rankings') { document.getElementById('main-content').classList.remove('hidden'); document.getElementById('key-metrics-overview').classList.remove('hidden'); renderUI(); }
        else if (view === 'loads') { document.getElementById('loads-analytics-content').classList.remove('hidden'); setTimeout(() => { renderLoadsAnalyticsUI(); initializeAnalyticsEventListeners(); }, 0); }
        else if (view === 'fleet-health') { document.getElementById('profiles-content').classList.remove('hidden'); renderTeamProfileUI(); }
