import { startTimer, fetchWithRetry, getPayrollWeekDateRange } from './utils.js';
...
import { openYear2025Modal } from './rankings/year_2025_ui.js';
...
import { renderDelegationUI } from './delegation/delegation_ui.js';
import { SettingsUI } from './performance/photos_url.js';
...
const refreshData = async (isInitialLoad = false) => {
    ...
    try {
        ...
        const settingsTimer = startTimer('fetchFleetHealthSettings');
        const overdueTimer = startTimer('fetchOverdueLoads');
        const photosTimer = startTimer('fetchPhotosSettings');
        ...
        const fetchPromises = [
            ...
            SettingsUI.init().then(() => { photosTimer.stop(); updateProgress(); console.log('[REFRESH] ✅ SettingsUI.init finished.'); }),
        ...
        try {
            renderTeamProfileUI();
            appState.rankingsComplianceCache = appState.profiles?.allProcessedDispatchers ? [...appState.profiles.allProcessedDispatchers] : [];
        } catch(e) {
            console.error(e);
        }
        ...
        // --- NEW: Dynamically build Contract Types from live and historical data ---
        const stubContracts = appState.allHistoricalData.flatMap(d => d.stubs || []).map(s => s.contractType).filter(Boolean);
        const allUniqueContracts = [...new Set(stubContracts.map(c => String(c).trim().toUpperCase()))].sort();
        
        if (allUniqueContracts.length > 0) {
            appState.loads.contractTypes = allUniqueContracts.map(c => ({ id: c, text: c }));
        } else {
            // Failsafe fallback
            appState.loads.contractTypes = [{ id: 'OO', text: 'OO' }, { id: 'LOO', text: 'LOO' }];
        }
        // ---------------------------------------------------------------------------
...
function finalizeSetup() {
    ...
    setTimeout(() => {
        ...
        // Preload Fleet Health loading image
        new Image().src = 'https://i.postimg.cc/tgjbpXkQ/FLEET-HEALTH-LOADING.png';
        ...
        if (snapshotTrigger && appState.currentView !== 'fleet-health') {
...
if (loadsMenuItem) {
    loadsMenuItem.style.display = 'none'; // Forcefully hide Loads tab
}
    ...
    const delegationMenuItem = document.getElementById('delegation-menu-item');
        if (delegationMenuItem) {
            ...
            if (hasDelegation) {
                delegationMenuItem.classList.remove('hidden');
                delegationMenuItem.style.display = '';
            } else {
                delegationMenuItem.classList.add('hidden');
            }
        }

        const dispatchersMenuItem = document.getElementById('dispatchers-menu-item');
        if (dispatchersMenuItem) {
            const hasPerformance = (user?.role === 'Admin') || (user?.permissions?.hasPerformanceAccess === true);
            if (hasPerformance) {
                dispatchersMenuItem.classList.remove('hidden');
                dispatchersMenuItem.style.display = '';
            } else {
                dispatchersMenuItem.classList.add('hidden');
            }
        }
...
const addEventListeners = () => {
    ...
    const switchView = async (view) => {
        ...
        if (view === 'fleet-health') {
            appState.profiles.selectedWeek = 'live';
        }
        if (view === 'dispatchers' && !(appState.auth.user?.role === 'Admin' || appState.auth.user?.permissions?.hasPerformanceAccess === true)) {
            console.warn("Access to Profiles denied.");
            return;
        }
        ...
        document.getElementById('delegation-content')?.classList.add('hidden');
        document.getElementById('dispatchers-view')?.classList.add('hidden'); 

        // Snapshot button logic
        const snapshotTrigger = document.getElementById('snapshot-trigger');
        if (snapshotTrigger && view !== 'fleet-health') { snapshotTrigger.style.display = 'none'; }
        ...
        if (view === 'rankings') { 
            ...
            const imgUrl = 'https://i.postimg.cc/tgjbpXkQ/FLEET-HEALTH-LOADING.png';
            
            container.innerHTML = `
                <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('${imgUrl}') center center / cover no-repeat; filter: blur(12px); opacity: 0.8; z-index: 1;"></div>
                <div style="position:absolute; top:0; left:0; width:100%; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; z-index:9999; color:white; font-family: 'Inter', sans-serif;">
                    ...
                    <div style="font-size:18px; font-weight:900; letter-spacing:6px; text-transform:uppercase; color:#fff; text-shadow: 0 4px 15px rgba(0,0,0,1);">Analyzing Fleet Health...</div>
            ...
            document.getElementById('dispatchers-view').classList.remove('hidden');
            DispatchersUI.init();
        }
   ...
   const dispatchersMenuItem = document.getElementById('dispatchers-menu-item');
    if (dispatchersMenuItem) {
        dispatchersMenuItem.addEventListener('click', (e) => {
            e.preventDefault();
            switchView('dispatchers');
        });
    }
    ...
    document.querySelectorAll('.ranking-mode-option').forEach(option => {
        option.addEventListener('click', (e) => {
            ...
            if (newMode && newMode !== appState.rankingMode) {
                ...
                if (newMode === 'team') {
                    appState.visibleColumns.team.set('numDispatchers', true);
                }
                fetchDataAndRender();
            }
        });
    ...
    document.getElementById('driver-type-switcher')?.addEventListener('click', async (e) => {
        const type = e.target.dataset.type;
        if (type && type !== appState.driverTypeFilter) {
            appState.driverTypeFilter = type;
            
            const mainContent = document.getElementById('main-content');
            if (mainContent) {
                let loader = document.getElementById('rankings-global-loader');
                if (!loader) {
                    loader = document.createElement('div');
                    loader.id = 'rankings-global-loader';
                    loader.className = 'fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-gray-900/50 backdrop-blur-sm transition-all duration-200';
                    loader.innerHTML = `
    ...
    document.getElementById('year-2025-btn')?.addEventListener('click', () => {
        openYear2025Modal();
    });
};
...
// --- GLOBAL HELPER: Open Dispatcher Profile ---
window.openDispatcherProfile = (name) => {
    if (typeof DispatchersUI !== 'undefined' && DispatchersUI.openDispatcherModal) {
        DispatchersUI.openDispatcherModal(name);
    } else {
        console.warn("DispatchersUI nije spreman ili nije definisan.");
    }
};