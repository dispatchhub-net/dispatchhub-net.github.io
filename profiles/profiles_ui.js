 ...
 * UPDATED: 1W Retention Logic (Deduplicated by Driver).
 * Pool: Unique Drivers with 'Active' + Unique Drivers with 'Terminated' status in the specific week.
 * Retained: Unique Drivers with 'Active' status.
 */
function calculateHistoricalRetention(stubsInWeek) {
    ...
    stubsInWeek.forEach(stub => {
        ...
        // 'Start' and 'Not_Start' are ignored for the pool
    });
 ...
 * UPDATED: 1W Live Prediction Logic.
 * Pool: Unique Drivers with 'Active' or 'Start' stub in the PREVIOUS week.
 * Lost: Drivers from the pool whose CURRENT Live Contract Status is 'Terminated'.
 */
function calculateLiveRetention(historicalStubs, liveContractStatus, liveDriverCounts, isDriverValidForViewFn) {
...
function calculateKpiData(baseData, isLiveData, allDrivers, historicalStubs, contractFilter, currentTeamData, weeksAgo) {
...
/**
 * Calculates the summary value (Median or Sum) for a given driver metric.
 */
        ...
        switch (col.id) {
            ...
            case 'flags':
                const flagsHTML = driver.flags.map(flag => {
                    ...
                    if (flag.tooltipData) {
                         tooltipAttr = `title="${flag.text} (Details)"`; 
                    }
...
/**
 * Renders the D3 Line Chart for Retention History.
 */
function renderRetentionHistoryChart(datasets) {
    ...
    datasets = datasets.filter(ds => !excludedTeams.includes(ds.name));

    // 2. Initialize hidden state: Show "All Teams" ONLY by default.
    datasets.forEach(ds => { 
    ...
    const getVisiblePoints = () => datasets.filter(ds => !ds.hidden).flatMap(ds => ds.data);
    ...
    if (visiblePoints.length > 0) {
        const dataMin = d3.min(visiblePoints, d => d.percentage);
        const dataMax = d3.max(visiblePoints, d => d.percentage);
        yMin = Math.max(0, dataMin - 10);
        yMax = Math.min(100, dataMax + 10);
    }

    const allPoints = datasets.flatMap(ds => ds.data);
    const x = d3.scaleTime()
        .domain(d3.extent(allPoints, d => d.weekDate))
        .range([0, width]);
    ...
    const colorScale = d3.scaleOrdinal([
        "#2dd4bf", "#f87171", "#60a5fa", "#fbbf24", "#a78bfa", "#34d399", "#f472b6", "#22d3ee", "#9ca3af", "#fb923c"
    ]);

    // --- AXES ---
    svg.append("g")
        .attr("transform", `translate(0,${height})`)
        .call(d3.axisBottom(x).ticks(12).tickFormat(d3.timeFormat("%b %d")))
        .attr("color", "#9ca3af")
        .attr("font-size", "10px");
    ...
    datasets.forEach((dataset, i) => {
        const isAggregate = dataset.name === "All Teams";
        const strokeWidth = isAggregate ? 4 : 2;
        const color = colorScale(i);

        // Line
        chartBody.append("path")
            .datum(dataset.data)
            .attr("class", `line-series line-${i}`)
            .attr("fill", "none")
            .attr("stroke", color)
            .attr("stroke-width", strokeWidth)
            .attr("d", line)
            .style("display", dataset.hidden ? "none" : null)
            .style("opacity", dataset.hidden ? 0 : 1);
    ...
    const legendContainer = container.append("div")
        .attr("class", "chart-legend flex flex-wrap justify-center gap-x-4 gap-y-2 mt-2 px-4 overflow-y-auto")
        .style("max-height", "80px"); 

    datasets.forEach((dataset, i) => {
        const color = colorScale(i);
        const item = legendContainer.append("div")
            .attr("class", `chart-legend-item cursor-pointer flex items-center gap-2 text-xs transition-all select-none ${dataset.hidden ? 'opacity-40 grayscale' : 'opacity-100'}`)
            .style("text-decoration", dataset.hidden ? "line-through" : "none")
            .on("click", function() {
                ...
                // Legend Style
                d3.select(this)
                    .classed("opacity-40 grayscale", isHidden)
                    .classed("opacity-100", !isHidden)
                    .style("text-decoration", isHidden ? "line-through" : "none");

                // Recalculate Scale
                const currentVisiblePoints = getVisiblePoints();
                let newYMin = 0;
                let newYMax = 100;
                if (currentVisiblePoints.length > 0) {
                    const dMin = d3.min(currentVisiblePoints, d => d.percentage);
                    const dMax = d3.max(currentVisiblePoints, d => d.percentage);
                    newYMin = Math.max(0, dMin - 10);
                    newYMax = Math.min(100, dMax + 10);
                }
                ...
                // Update Axis & Grid
                yAxisGroup.transition(t).call(d3.axisLeft(y).ticks(5).tickFormat(d => d + "%"));
                gridGroup.transition(t).call(d3.axisLeft(y).tickSize(-width).tickFormat("").ticks(5));

                // Update Lines that are STAYING visible (smooth transition)
                chartBody.selectAll(".line-series")
                    .filter(function() { 
                        // Don't transition the geometry of the one being TOGGLED ON here, 
                        // we handle it specifically below to avoid morphing from wrong coordinates
                        return this !== chartBody.select(`.line-${i}`).node(); 
                    })
                    .transition(t)
                    .attr("d", function(d) { return line(d); });

                // Update Dots (Global)
                chartBody.selectAll(".dot-series")
                    .transition(t)
                    .attr("cy", d => y(d.percentage));

                // Handle Toggled Line Specifics
                const toggledLine = chartBody.select(`.line-${i}`);
                const toggledDots = chartBody.selectAll(`.dot-${i}`);

                if (isHidden) {
                    // Hide: Morph to new scale AND fade out
                    toggledLine.transition(t)
                        .attr("d", function(d) { return line(d); })
                        .style("opacity", 0)
                        .on("end", function() { d3.select(this).style("display", "none"); });
                    
                    toggledDots.transition(t)
                        .attr("cy", d => y(d.percentage))
                        .style("opacity", 0)
                        .on("end", function() { d3.select(this).style("display", "none"); });
                } else {
                    // Show: Snap geometry immediately, then fade in
                    toggledLine
                        .style("display", null)
                        .attr("d", function(d) { return line(d); }) // Snap to correct shape
                        .style("opacity", 0)
                    ...
                    toggledDots
                        .style("display", null)
                        .attr("cy", d => y(d.percentage)) // Snap to correct position
                        .style("opacity", 0)
                        .transition().duration(750)
                        .style("opacity", 1);
                }
        ...
        item.append("div").attr("class", "w-3 h-3 rounded-full flex-shrink-0").style("background-color", color);
        item.append("span").attr("class", "text-gray-300 font-medium").text(dataset.name);
    });

    // --- TOOLTIP ---
    const tooltip = d3.select("body").append("div")
        .attr("class", "d3-tooltip retention-tooltip")
        .style("opacity", 0);
    ...
    svg.append("rect")
        ...
        .on("mouseout", () => { focus.style("display", "none"); tooltip.style("opacity", 0); })
        .on("mousemove", (event) => {
            const visibleDatasets = datasets.map((ds, i) => ({...ds, originalIndex: i})).filter(ds => !ds.hidden);
            if (visibleDatasets.length === 0) { tooltip.style("opacity", 0); focus.style("display", "none"); return; }

            const x0 = x.invert(d3.pointer(event)[0]);
            
            const bisect = d3.bisector(d => d.weekDate).left;
            const referenceData = visibleDatasets[0].data;
            const i = bisect(referenceData, x0, 1);
            ...
            if(d) {
                ...
                let tooltipHtml = `<div class="font-bold text-white border-b border-gray-600 pb-1 mb-1">${d3.timeFormat("%b %d")(d.weekDate)}</div>`;
                
                const sortedPoints = visibleDatasets.map(ds => {
                    const point = ds.data.find(p => p.weekDate.getTime() === d.weekDate.getTime());
                    return point ? { name: ds.name, ...point, color: colorScale(ds.originalIndex) } : null;
                }).filter(p => p && p.total > 0).sort((a, b) => b.percentage - a.percentage);

                sortedPoints.forEach(pt => {
                    tooltipHtml += `<div class="flex justify-between items-center gap-4 text-xs mb-1"><span style="color:${pt.color}" class="font-semibold">${pt.name}</span><div class="text-right"><span class="text-white font-mono">${pt.percentage.toFixed(1)}%</span><span class="text-gray-500 ml-1">(${pt.active}/${pt.total})</span></div></div>`;
                });
