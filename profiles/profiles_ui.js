...
import { DispatchersUI } from '../performance/performance_ui.js';
...
const SILHOUETTE_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4"><path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule='evenodd' /></svg>`;
...
function calculateLiveFlagsForDriver(driverName, historicalStubs, allDriversData) {
    ...
    if (settings.highTolls.enabled) {
        ...
        if (relevantStubsForDriver.length >= settings.highTolls.minStubs) { 
            const relevantPayDates = new Set(relevantStubsForDriver.map(rs => rs.pay_date));
            const allRelevantStubs = historicalStubs.filter(stub => 
                relevantPayDates.has(stub.pay_date) && stub.total_miles && stub.total_miles > 0
            );
...
const dispatchTableColumns = [
    ...
    { id: 'tpogTrucks', label: '(T)POG Trucks', type: 'number' },
    { id: 'cpmTrucks', label: 'CPM Trucks', type: 'number' },
...
export function renderDriverDeepDiveModal_Profiles() {
    ...
    if (isModalOpen && selectedDriver) {
        ...
        if (headerRight) {
            ...
            const isMarginHidden = appState.profiles.driverDeepDive.hideMarginColumn || false;
            const isContractHidden = appState.profiles.driverDeepDive.hideContractColumn || false;

            const existingDropdown = document.getElementById('driver-deep-dive-settings-dropdown');
            
            if (!existingDropdown) {
                headerRight.innerHTML = `
                    <div class="relative" id="driver-deep-dive-settings-container">
                        <button id="driver-deep-dive-settings-btn" class="text-gray-400 hover:text-white transition-colors p-1 mt-1 flex items-center justify-center">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75" /></svg>
                        </button>
                        <div id="driver-deep-dive-settings-dropdown" class="absolute right-0 mt-2 w-auto min-w-[200px] bg-gray-800 border border-gray-600 rounded-lg shadow-xl z-50 hidden cursor-default">
                             <div class="flex flex-col gap-1 p-2 text-sm text-gray-200">
                                <label class="flex items-center gap-3 cursor-pointer hover:bg-gray-700/50 p-2 rounded transition-colors select-none">
                                    <input type="checkbox" id="canceled-loads-checkbox" class="form-checkbox h-4 w-4 text-teal-500 rounded border-gray-500 bg-gray-700 focus:ring-teal-500 focus:ring-offset-gray-800 transition duration-150 ease-in-out" ${showCanceledLoads ? 'checked' : ''}>
                                    <span class="font-medium text-center flex-grow">Canceled Loads</span>
                                </label>
                                
                                <label class="flex items-center gap-3 cursor-pointer hover:bg-gray-700/50 p-2 rounded transition-colors select-none">
                                    <input type="checkbox" id="margin-column-checkbox" class="form-checkbox h-4 w-4 text-teal-500 rounded border-gray-500 bg-gray-700 focus:ring-teal-500 focus:ring-offset-gray-800 transition duration-150 ease-in-out" ${!isMarginHidden ? 'checked' : ''}>
                                    <span class="font-medium text-center flex-grow">Margin Column</span>
                                </label>

                                <label class="flex items-center gap-3 cursor-pointer hover:bg-gray-700/50 p-2 rounded transition-colors select-none">
                                    <input type="checkbox" id="contract-type-column-checkbox" class="form-checkbox h-4 w-4 text-teal-500 rounded border-gray-500 bg-gray-700 focus:ring-teal-500 focus:ring-offset-gray-800 transition duration-150 ease-in-out" ${!isContractHidden ? 'checked' : ''}>
                                    <span class="font-medium text-center flex-grow">Contract Type</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    <button id="close-profiles-deep-dive-modal-btn" class="text-gray-400 hover:text-white transition-colors text-3xl ml-2">&times;</button>
                `;
            } else {
                const canceledBox = document.getElementById('canceled-loads-checkbox');
                const marginBox = document.getElementById('margin-column-checkbox');
                const contractBox = document.getElementById('contract-type-column-checkbox');
                if (canceledBox) canceledBox.checked = showCanceledLoads;
                if (marginBox) marginBox.checked = !isMarginHidden;
                if (contractBox) contractBox.checked = !isContractHidden;
            }
        }
        ...
        const fullContractType = mostRecentContractStub.contract_type || contractType || 'UNKNOWN';
        ...
        if (isLiveData) {
            const driverLoads = appState.profiles.liveData
                .filter(l => l.driver === selectedDriver)
                .sort((a, b) => new Date(b.do_date) - new Date(a.do_date));
                
            const loadWithTeam = driverLoads.find(l => l.status_teams);
            if (loadWithTeam) teamStatus = loadWithTeam.status_teams;
            
            const loadWithFranchise = driverLoads.find(l => l.franchise_name);
            if (loadWithFranchise) franchiseStatus = loadWithFranchise.franchise_name;
        } else {
            const recentStubWithStatus = historicalStubs.find(s => s.status_teams);
            if (recentStubWithStatus) teamStatus = recentStubWithStatus.status_teams;
            
            const recentStubWithFranchise = historicalStubs.find(s => s.franchise_name);
            if (recentStubWithFranchise) franchiseStatus = recentStubWithFranchise.franchise_name;
        }

        // Render modal components
        renderModalHeader_Profiles(selectedDriver, historicalStubs, contractType, teamStatus, fullContractType, franchiseStatus);
...
function renderModalHeader_Profiles(driverName, historicalStubs, contractType, teamStatus, fullContractType, franchiseStatus) {
    ...
    const displayContract = (contractType !== fullContractType && fullContractType !== 'UNKNOWN')
...
stubsData.forEach(stub => {
    const dateKey = stub.pay_date ? new Date(stub.pay_date).toISOString().split('T')[0] : 'predicted_' + stub.weeks_ago;
    if (!dateGroups[dateKey]) dateGroups[dateKey] = [];
    dateGroups[dateKey].push(stub);
});
stubsData = Object.values(dateGroups).map(group => {
    if (group.length === 1) return group[0];
    const withMiles = group.filter(s => parseFloat(s.total_miles) > 0);
    if (withMiles.length > 0) {
        const merged = { ...withMiles[0] };
        for (let i = 1; i < withMiles.length; i++) {
            const s = withMiles[i];
            merged.driver_gross = (parseFloat(merged.driver_gross) || 0) + (parseFloat(s.driver_gross) || 0);
            merged.margin = (parseFloat(merged.margin) || 0) + (parseFloat(s.margin) || 0);
            merged.net_pay = (parseFloat(merged.net_pay) || 0) + (parseFloat(s.net_pay) || 0);
            merged.total_miles = (parseFloat(merged.total_miles) || 0) + (parseFloat(s.total_miles) || 0);
            merged.fuel_used = (parseFloat(merged.fuel_used) || 0) + (parseFloat(s.fuel_used) || 0);
            merged.escrow_paid = (parseFloat(merged.escrow_paid) || 0) + (parseFloat(s.escrow_paid) || 0);
            merged.balance = (parseFloat(merged.balance) || 0) + (parseFloat(s.balance) || 0);
            merged.balance_settle = (parseFloat(merged.balance_settle) || 0) + (parseFloat(s.balance_settle) || 0);
            merged.po_settle = (parseFloat(merged.po_settle) || 0) + (parseFloat(s.po_settle) || 0);
            merged.rpm_all = merged.total_miles > 0 ? merged.driver_gross / merged.total_miles : 0;
            if (merged.stub_dispatcher !== s.stub_dispatcher && s.stub_dispatcher) merged.stub_dispatcher += ' / ' + s.stub_dispatcher;
            if (merged.stub_team !== s.stub_team && s.stub_team) merged.stub_team += ' / ' + s.stub_team;
        }
        return merged;
    }
    return group[0];
...
const payDateCounts = {};
    ...
    const headers = [
        ...
        ...(appState.profiles.driverDeepDive.hideContractColumn ? [] : [{ label: 'Contract Type', key: 'contract_type', format: (d) => d === null || d === undefined ? '-' : d, class: 'text-sky-300 font-bold' }]),
        {
            label: 'Contract',
            key: 'contract',
            format: (d, stub) => {
                const cType = (stub.contract_type || '').toUpperCase();
                if (cType === 'TPOG') {
                    const dGross = parseFloat(stub.driver_gross) || 0;
                    const gross_pay = parseFloat(stub.gross_pay) || 0;
                    if (dGross === 0) return '-';
                    const calc = (100 / dGross) * gross_pay;
                    return `${calc.toFixed(1)}%`;
                }
                if (d === null || d === undefined) return '-';
                return formatPercentage(parseFloat(d));
            },
            class: 'text-sky-400'
        },
        ...
        ...(appState.profiles.driverDeepDive.hideMarginColumn ? [] : [{ label: 'Margin', key: 'margin', format: (d) => d === '-' ? '-' : `$${(parseFloat(d) || 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}`, class: 'text-yellow-400' }]),
        {
            label: 'Net',
            key: 'net_pay',
            format: (d, stub) => {
        ...
        },
        {
            label: 'Escrow',
            key: 'escrow_paid',
            format: (d) => d === '-' ? '-' : `$${(parseFloat(d) || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            class: 'text-gray-300'
        },
        { label: 'Miles', key: 'total_miles', format: (val) => val === '-' ? '-' : (parseFloat(val) || 0).toLocaleString() },
        { label: 'RMP (ALL)', key: 'rpm_all', format: (d) => d === '-' ? '-' : `$${(parseFloat(d) || 0).toFixed(2)}` },
        { label: 'Dispatch', key: 'stub_dispatcher' },
        { label: 'Team', key: 'stub_team' },
        { label: 'Franchise', key: 'franchise_name' },
        {
        ...
        tableBody.addEventListener('click', e => {
            ...
            if (loadsForStubDrilldown.length > 0) {
                const loadsTable = loadsForStubDrilldown.map(load => {
                    ...
                    const loadRPM = totalMiles > 0 ? (load.price || 0) / totalMiles : 0;
                    ...
                    return `
                        <tr class="hover:bg-cyan-700/50 ${isCanceled ? 'opacity-50' : ''}">
                            ...
                            ${!appState.profiles.driverDeepDive.hideMarginColumn ? `<td class="px-2 py-1.5 text-right font-mono text-yellow-400">$${(load.cut || 0).toLocaleString()}</td>` : ''}
                        ...
                        <table class="w-full text-xs">
                            <thead>
                                <tr class="border-b border-cyan-700">
                                    ...
                                    <th class="px-2 py-2 text-right">Driver Rate</th>
                                    ${!appState.profiles.driverDeepDive.hideMarginColumn ? '<th class="px-2 py-2 text-right">Margin</th>' : ''}
                                    <th class="px-2 py-2 text-right">RMP (ALL)</th><th class="px-2 py-2 text-right">Miles</th>
...
function renderDriverSettingsModal() {
    ...
    const createThresholdsWithOverrides = (flagKey, thresholdKey, labelText, step = "1", showLabel = true) => {
        ...
        const allContracts = appState.loads?.contractTypes?.map(c => c.id) || ['OO', 'LOO'];
        const availableOverrides = allContracts.filter(c => !Object.keys(thresholds.by_contract).includes(c));
        return `<div class="input-group compact">
...
export const renderTeamProfileUI = async () => {
    if (appState.profiles.selectedWeek === 'live' && appState.profiles.savedWeek && appState.profiles.savedWeek !== 'live') {
        appState.profiles.selectedWeek = appState.profiles.savedWeek;
    }
    ...
    if (snapshotTrigger) {
        ...
        if (appState.currentView === 'fleet-health' && canViewSnapshot() && isSpecificTeamSelected && isHistoricalWeekSelected && isAllCompaniesSelected && isAllFranchisesSelected) {
    ...
    const profilesContent = document.getElementById('profiles-content');
        const isHeadless = !profilesContent;

        if (!appState.profiles.liveData || appState.profiles.liveData.length === 0) {
            if (!isHeadless) {
                profilesContent.innerHTML = `<div class="flex items-center justify-center h-full"><div class="text-center"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-teal-400 mx-auto"></div><div style="font-family: 'Inter', sans-serif; font-size: 18px; font-weight: 700; letter-spacing: 6px; text-transform: uppercase; color: #fff; text-shadow: 0 4px 15px rgba(0, 0, 0, 1); margin-top: 25px;">Loading Live Profile Data...</div></div></div>`;
            }
            return;
        }

        if (!isHeadless && document.getElementById('profiles-header')) {
            profilesContent.style.position = 'relative';
            let loader = document.getElementById('profiles-global-loader');
            if (!loader) {
                loader = document.createElement('div');
                loader.id = 'profiles-global-loader';
                loader.className = 'absolute inset-0 z-[100] flex flex-col items-center justify-center bg-gray-900/50 backdrop-blur-sm rounded-lg transition-all duration-200';
                loader.innerHTML = `
                    <div class="animate-spin rounded-full h-14 w-14 border-b-4 border-teal-400"></div>
                    <div class="mt-4 text-teal-400 font-bold tracking-widest uppercase text-sm drop-shadow-md">Loading...</div>
                `;
                profilesContent.appendChild(loader);
            }
    ...
    if (!appState.selectedFleetHealthDate) {
        appState.selectedFleetHealthDate = appState.selectedDate;
    }
   ...
   if (selectedWeek !== 'live' && appState.profiles.fleetHealthCache[cacheKey]) {
            ...
            appState.profiles.prevWeekKpis = prevWeekKpis;
    ...
    const filterDataByDateAndTeam = (source, start, end, isForLiveData, ignoreFilters = false) => {
        let dateFilteredSource;
        if (isForLiveData) {
            dateFilteredSource = source.filter(d => d.do_date && new Date(d.do_date) >= start && new Date(d.do_date) <= end);
        } else {
            // Determine Target Pay Date (Standard vs Delayed)
            const stdDate = new Date(end);
            stdDate.setUTCDate(end.getUTCDate() + 3);
            const stdDateStr = stdDate.toISOString().split('T')[0];
            
            const delDate = new Date(end);
            delDate.setUTCDate(end.getUTCDate() + 10);
            const delDateStr = delDate.toISOString().split('T')[0];

            // Check if Standard Date has data in source. If not, try Delayed.
            const hasStd = source.some(d => d.pay_date && new Date(d.pay_date).toISOString().split('T')[0] === stdDateStr);
            const targetStr = hasStd ? stdDateStr : delDateStr;

            dateFilteredSource = source.filter(d => {
                if (!d.pay_date) return false;
                return new Date(d.pay_date).toISOString().split('T')[0] === targetStr;
            });
        }
    ...
    let processedDrivers = allDriversInViewUnfiltered.map((name, index) => {
        ...
        const contractType = firstEntry.contract_type ? firstEntry.contract_type.toUpperCase() : 'UNKNOWN';
    ...
    if (contractTypeFilter !== 'all') {
        processedDrivers = processedDrivers.filter(d => d.contract.toUpperCase() === contractTypeFilter.toUpperCase());
    }
    ...
    const masterDispatcherList = allDispatcherNamesForCurrentWeek.map((name, index) => {
        ...
        const ooTrucks = liveDriversForDispatcher.filter(d => (d.contract_type || 'UNKNOWN').toUpperCase() === 'OO').length;
        const looTrucks = liveDriversForDispatcher.filter(d => (d.contract_type || 'UNKNOWN').toUpperCase() === 'LOO').length;
        const tpogTrucks = liveDriversForDispatcher.filter(d => ['POG', 'TPOG'].includes((d.contract_type || 'UNKNOWN').toUpperCase())).length;
        const cpmTrucks = liveDriversForDispatcher.filter(d => (d.contract_type || 'UNKNOWN').toUpperCase() === 'CPM').length;
        ...
        return {
            ...
            allTrucks, ooTrucks, looTrucks, tpogTrucks, cpmTrucks,
 ...
 // Calculator Activity Logic
            ...(() => {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    const record = activityData.find(d => d.dispatcher === name && new Date(d.date).toISOString().split('T')[0] === dayString);
                    const rawValue = record ? String(record.minutes).trim().toUpperCase() : 'X';
                    const minutesVal = (rawValue !== 'X' && rawValue !== '') ? (parseFloat(rawValue) || 0) : 0;
                    const hasVisit = minutesVal > 0;
                    
                    if (hasVisit) visits++;
                    activityDetails.push({ date: day, minutes: minutesVal, isX: !hasVisit });
                }
                return {
                    calculatorActivity: (visits / 7) * 100,
                    calculatorActivityCount: visits,
   ...
   // --- START: NEW POINT-IN-TIME MEDIAN TENURE LOGIC ---
    ...
    masterDispatcherList.forEach(dispatcher => {
        ...
        const dispatcherDriversInView = globalDriversForTenure
            ...
            const getTenureList = (contractType) => {
                const drivers = dispatcherDriversInView.filter(d => d.contract === contractType);
    ...
    if (contractTypeFilter !== 'all') {
        const targetContract = contractTypeFilter.toUpperCase();
        dispatchersToDisplay = dispatchersToDisplay.filter(d => {
            // Check if the dispatcher has any recent loads matching the contract type
            const hasLoadContract = d.loads && d.loads.some(l => (l.contract_type || 'UNKNOWN').toUpperCase() === targetContract);
            
            // Check if they currently have active drivers matching the contract type
            const liveCounts = appState.profiles.liveDriverCountData || [];
            const hasLiveContract = liveCounts.some(ld => 
                ld.dispatcher_name === d.name && 
                (ld.contract_type || 'UNKNOWN').toUpperCase() === targetContract
            );
            
            return hasLoadContract || hasLiveContract;
        });
    }
    if (selectedCompany && selectedCompany !== 'All Companies') {
       ...
       const calculatorActivity = (prevDaysVisited / 7) * 100;
        ...
        const getTenureList = (contractType) => {
            ...
            const driversWithStubs = enrichedHistoricalStubs.filter(s => 
                ...
                String(s.contract_type || 'UNKNOWN').toUpperCase() === contractType &&
        ...
        return {
            ...
            missingPaperwork,
            calculatorActivity,
            calculatorActivityCount: prevDaysVisited,
            rcEntry,
    ...
    appState.profiles.allProcessedDispatchers_PREV = allProcessedDispatchers_PREV;

    if (selectedWeek === 'live') {
        appState.profiles.cachedWeek1HighFidelity = allProcessedDispatchers_PREV;
    }
    ...
    if (contractTypeFilter !== 'all') {
        ...
        dispatchersToDisplay_PREV = dispatchersToDisplay_PREV.filter(disp => {
            ...
            return liveInfo.some(d => (d.contract_type || 'UNKNOWN').toUpperCase() === contractTypeFilter.toUpperCase());
        });
    }
    ...
    appState.profiles.currentKpis = currentKpis; 
    if (selectedWeek === 'live') {
        appState.profiles.cachedLiveKpis = currentKpis;
    }
    appState.profiles.prevWeekKpis = prevWeekKpis;
...
function renderDispatchTable(dispatchersToDisplay, allDispatchers, selectedWeek) {
                                ...
                                switch(col.type) {
                                    ...
                                    default:
                                        if (col.id === 'name') {
                                            ...
                                            const isInPerformance = window.DispatchersUI?.validPerformanceNames?.has(cleanName);
                                            
                                            const profileIcon = (isAdmin() && isInPerformance) ? 
                                                `<div onclick="event.stopPropagation(); window.openDispatcherProfile('${d.name}')" class="cursor-pointer text-gray-500 hover:text-teal-400 transition-colors opacity-0 group-hover:opacity-100" title="View Profile">
                                                    ${SILHOUETTE_ICON_SVG}
                                                </div>` : '';

                                            content = `<div class="flex items-center gap-2 group">
                                                ${d.name}
                                                ${profileIcon}
                                            </div>`;
                                        } else if (col.id === 'medianTenureOO' || col.id === 'medianTenureLOO') {
                                            ...
                                            const tenureData = driversForDispatcher
                                                // FILTER FOR OO or LOO based on the column ID (Note: If expanding columns to other contracts, adjust this logic to match col.id dynamically)
                                                .filter(driver => {
                                                    if (col.id === 'medianTenureOO') return driver.contract === 'OO';
                                                    if (col.id === 'medianTenureLOO') return driver.contract === 'LOO';
                                                    return true; 
                                                })
                                                .map(driver => {
    ...
    const newTableContainer = document.getElementById('profiles-dispatch-table-container');
...
export function initializeProfileEventListeners() {
    if (!document.body.profileListenersAttached) {
        ...
        document.body.addEventListener('click', (e) => {
            ...
            if (Object.values(openDropdowns).some(isOpen => isOpen) && !isClickInsideAnyDropdown) {
                ...
                if (typeof renderKpiSettingsDropdown === 'function') renderKpiSettingsDropdown();
                if (typeof renderContractTypeFilterDropdown === 'function') renderContractTypeFilterDropdown();
                if (typeof renderCompanyFilterDropdown === 'function') renderCompanyFilterDropdown();
                if (typeof renderFranchiseFilterDropdown === 'function') renderFranchiseFilterDropdown();
                if (typeof renderProfileWeekSelector === 'function') renderProfileWeekSelector();
            }
        ...
        header._clickHandler = (e) => {
            ...
            if (!targetId) {
                ...
                if (weekOption) {
                    ...
                    appState.profiles.savedWeek = weekOption.dataset.weekId;
                    appState.profiles.isWeekSelectorOpen = false;
                    
                    const dropdown = document.getElementById('profile-week-selector-dropdown');
                    if (dropdown) dropdown.remove();
                    
                    const profilesContent = document.getElementById('profiles-content');
                    if (profilesContent) {
                        profilesContent.style.opacity = '0.5';
                        profilesContent.style.pointerEvents = 'none';
                    }

                    setTimeout(() => {
                        renderTeamProfileUI();
                        if (profilesContent) {
                            profilesContent.style.opacity = '1';
                            profilesContent.style.pointerEvents = 'auto';
                        }
                    }, 10);
                }
                return;
            ...
            const dropdownStates = {
                'kpi-settings-btn': { stateKey: 'isKpiSettingsOpen', renderFn: () => { if (typeof renderKpiSettingsDropdown === 'function') renderKpiSettingsDropdown(); } },
                'contract-type-filter-btn': { stateKey: 'isContractTypeFilterOpen', renderFn: renderContractTypeFilterDropdown },
                'company-filter-btn': { stateKey: 'isCompanyFilterOpen', renderFn: renderCompanyFilterDropdown },
                'franchise-filter-btn': { stateKey: 'isFranchiseFilterOpen', renderFn: renderFranchiseFilterDropdown },
                'profile-week-selector-btn': { stateKey: 'isWeekSelectorOpen', renderFn: () => { if (typeof renderProfileWeekSelector === 'function') renderProfileWeekSelector(); } }
            };

            const clickedConfig = dropdownStates[targetId];
            if (!clickedConfig) return;

            const wasOpen = appState.profiles[clickedConfig.stateKey];

            Object.values(dropdownStates).forEach(config => {
                appState.profiles[config.stateKey] = false;
                config.renderFn();
            });
            
            appState.profiles[clickedConfig.stateKey] = !wasOpen;
            clickedConfig.renderFn();
        };
    ...
    if (dispatchTable) {
        ...
        dispatchTable._clickHandler = (e) => {
            ...
            if (tooltipTrigger) {
                ...
                dispatchTable.querySelectorAll('.dispatch-table-row').forEach(r => {
                    r.classList.toggle('selected', parseInt(r.dataset.dispatcherId, 10) === appState.profiles.selectedDispatcherId);
                });
                ...
                requestAnimationFrame(() => {
                    const newRow = dispatchTable.querySelector(`.dispatch-table-row[data-dispatcher-id='${dispatcherId}']`);
                    if (newRow && appState.profiles.selectedDispatcherId === dispatcherId) {
    ...
    if (driverDeepDiveModal && !driverDeepDiveModal._clickHandlerAttached) {
        driverDeepDiveModal.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#driver-deep-dive-settings-btn')) {
                const dropdown = document.getElementById('driver-deep-dive-settings-dropdown');
                if (dropdown) dropdown.classList.toggle('hidden');
            } else if (!e.target.closest('#driver-deep-dive-settings-container')) {
                const dropdown = document.getElementById('driver-deep-dive-settings-dropdown');
                if (dropdown) dropdown.classList.add('hidden');
            }

            if (e.target.id === 'canceled-loads-checkbox') {
                appState.profiles.driverDeepDive.showCanceledLoads = e.target.checked;
                renderDriverDeepDiveModal_Profiles();
            }
            
            if (e.target.id === 'margin-column-checkbox') {
                appState.profiles.driverDeepDive.hideMarginColumn = !e.target.checked;
                renderDriverDeepDiveModal_Profiles();
            }
            
            if (e.target.id === 'contract-type-column-checkbox') {
                appState.profiles.driverDeepDive.hideContractColumn = !e.target.checked;
                renderDriverDeepDiveModal_Profiles();
            }
        });
...
function renderDriverFilterModal() {
    ...
    const filterColumns = [
        ...
        { id: 'contract', label: 'Contract', type: 'select', options: (appState.loads?.contractTypes?.map(c => c.id) || ['OO', 'LOO']) },
...
function renderContractTypeFilterDropdown() {
    ...
    const dynamicTypes = appState.loads?.contractTypes || [];
    const options = [
        { id: 'all', label: 'All Drivers' },
        ...dynamicTypes.map(c => ({ id: c.id.toLowerCase(), label: `${c.id} Only` }))
    ];
...
function renderCompanyFilterDropdown() {
    ...
    // 3. Extract unique companies from the final dataset AND historical stubs to get all possible options.
    const historicalStubs = appState.loads.historicalStubsData || [];
    
    // First, try to get companies from the currently filtered view (e.g. if a specific team is selected)
    let companies = [...new Set(relevantData.map(d => d.company_name).filter(Boolean))];
    
    // If the list is empty or we want to ensure all companies are available for the selected team/globally
    if (companies.length === 0 || selectedTeam === 'ALL_TEAMS') {
         // Gather all companies from live data
        const liveCompanies = (appState.profiles.liveData || []).map(d => d.company_name).filter(Boolean);
        // Gather all companies from historical stubs
        const historicalCompanies = historicalStubs.map(s => s.company_name).filter(Boolean);
        
        // Combine, filter out empty strings/nulls, and get unique values
        const allPossibleCompanies = [...new Set([...liveCompanies, ...historicalCompanies])];
        
        // If a specific team is selected, we might want to filter the historical companies by that team
        if (selectedTeam !== 'ALL_TEAMS') {
            const teamLower = selectedTeam.toLowerCase();
            const specialPrefixes = ['agnius', 'uros', 'miles', 'wayne', 'mubeen', 'vito'];
            
            companies = [...new Set(historicalStubs.filter(s => {
                if (!s.stub_team) return false;
                if (specialPrefixes.includes(teamLower)) {
                    return s.stub_team.toLowerCase().startsWith(teamLower);
                }
                return s.stub_team === selectedTeam;
            }).map(s => s.company_name).filter(Boolean))];
            
            // Merge with whatever we found in live data for this team
            companies = [...new Set([...companies, ...relevantData.map(d => d.company_name).filter(Boolean)])];

        } else {
             companies = allPossibleCompanies;
        }
    }
    
    companies = [...new Set(companies.map(c => c.trim()))]
        .filter(c => c.toLowerCase() !== 'demo')
        .sort();
...
function generateSnapshotData() {
   ...
   const { dispatchers, drivers } = currentTeamData;
    ...
    let rankedDispatchers = [...dispatchers];
    const hasOfficialRanks = rankedDispatchers.some(d => d.rank1w !== null && d.rank1w !== undefined);
    
    if (!hasOfficialRanks) {
        rankedDispatchers.sort((a, b) => (b.complianceScore || 0) - (a.complianceScore || 0));
        rankedDispatchers.forEach((d, i) => d.tempRank = i + 1);
    } else {
        rankedDispatchers = rankedDispatchers.filter(d => d.rank1w !== null && d.rank1w !== undefined);
        rankedDispatchers.sort((a, b) => a.rank1w - b.rank1w);
        rankedDispatchers.forEach(d => d.tempRank = d.rank1w);
    }

    const bestDispatchersRank = rankedDispatchers.slice(0, 3);
    const worstDispatchersRank = rankedDispatchers.slice(-3).reverse();
...
export function renderSnapshotChart(containerId) {
   ...
   const uniqueDates = [...new Set(chartData.map(d => d.date.getTime()))].sort((a, b) => a - b).map(t => new Date(t));
    ...
    const legendContainer = d3.select("#snapshot-chart-legend-container");
    legendContainer.html(""); // Clear previous legend
    if (datasets.length > 1) {
        datasets.forEach((ds, i) => {
            legendContainer.append("div")
                .attr("class", "flex items-center gap-1.5 text-xs text-gray-300")
                .html(`<div class="w-2.5 h-2.5 rounded-full" style="background-color: ${colorScale(i)};"></div> ${ds.name}`);
        });
    }
...
export function renderTeamSnapshot() {
    ...
    content.innerHTML = `
        ...
        <div class="snapshot-section flex-grow min-h-0 flex flex-col">
             <div class="flex items-center justify-between mb-2">
                 <h3 class="snapshot-section-title !mb-0">Weekly Rankings (1w)</h3>
                 <div id="snapshot-chart-legend-container" class="flex gap-4 items-center"></div>
             </div>
             <div class="snapshot-chart-container flex-grow relative" id="snapshot-chart-container"></div>
        </div>
        ...
        <div class="snapshot-section">
            ...
            <div class="snapshot-rankings-grid">
                <div>
                    <h5>Top 3 (Rank)</h5>
                    ${bestDispatchersRank.map(d => `<div class="snapshot-list-item"><span class="name">${d.name}</span><span class="value good">#${d.tempRank || '-'}</span></div>`).join('')}
                </div>
                <div>
                    <h5>Bottom 3 (Rank)</h5>
                    ${worstDispatchersRank.map(d => `<div class="snapshot-list-item"><span class="name">${d.name}</span><span class="value bad">#${d.tempRank || '-'}</span></div>`).join('')}
                </div>
                 <div>
...
function calculateRetentionHistoryData() {
    ...
    const calculateTrendForScope = (scopeName, teamFilter) => {
        ...
        // 1. Filter Stubs for this Scope (Team/Context)
        const isStubValid = (s) => {
            ...
            if (contractTypeFilter !== 'all') {
                ...
                if (raw !== contractTypeFilter.toUpperCase()) return false;
           }
            return true;
        };

        // 2. Filter Live Drivers for this Scope
        ...
        // --- Helpers definition START ---
        const dateHasStubs = (dateStr) => scopedStubs.some(s => s.pay_date && new Date(s.pay_date).toISOString().split('T')[0] === dateStr);

        const getBestDateForWeek = (weekEnd) => {
            const std = new Date(weekEnd); 
            std.setUTCDate(weekEnd.getUTCDate() + 3);
            const stdStr = std.toISOString().split('T')[0];
            
            const del = new Date(weekEnd); 
            del.setUTCDate(weekEnd.getUTCDate() + 10);
            const delStr = del.toISOString().split('T')[0];

            return (dateHasStubs(stdStr) || !dateHasStubs(delStr)) ? stdStr : delStr;
        };
        // --- Helpers definition END ---

        for (let i = weeksToCheck; i >= 0; i--) {
            const { end } = getPayrollWeekDateRange(i);
            const targetDateString = getBestDateForWeek(end);
            ...
            // Logic: If stubs exist, use historical. If missing (Live or recent weeks w/o payroll), fallback to Live logic.
            const useLiveLogic = i === 0 || (i <= 2 && stubsInWeek.length === 0);

            if (useLiveLogic) {
                // Attempt to find a valid historical pool date by looking back
                let poolDate = null;
                for (let k = i + 1; k <= weeksToCheck + 4; k++) { 
                    const { end: pEnd } = getPayrollWeekDateRange(k);
                    const pDate = getBestDateForWeek(pEnd);
                    if (dateHasStubs(pDate)) {
                        poolDate = pDate;
                        break;
                    }
                }
                ...
                if (poolDate) {
                    ...
                    result = { percentage: 0, active: 0, total: 0, terminated: 0, transferred: 0 };
                }

                const kpiSource = (i === 0 && appState.profiles.cachedLiveKpis) ? appState.profiles.cachedLiveKpis : appState.profiles.currentKpis;
            
            if (i === 0 && teamFilter === 'ALL_TEAMS' && kpiSource && kpiSource.medianRetentionOptimistic > 0) {
                const rc = kpiSource.retentionCounts || {};
                result = {
                    percentage: kpiSource.medianRetentionOptimistic,
                        percentage: appState.profiles.currentKpis.medianRetentionOptimistic, 
                        active: rc.active || 0, 
                        total: rc.total || 0, 
                        terminated: rc.terminated || 0, 
                        transferred: rc.transferred || 0 
                    };
                }
            } else {
            ...
            historyData.push({ weekDate: end, retention: result.percentage, active: result.active, total: result.total });
        }

        return { name: scopeName, data: historyData.sort((a,b) => a.weekDate - b.weekDate) };
    };

    let datasets = [calculateTrendForScope('All Teams', 'ALL_TEAMS')];
    
    const uniqueTeams = [...new Set(appState.profiles.liveData.map(d => d.team).filter(Boolean))];
    uniqueTeams.forEach(team => {
        datasets.push(calculateTrendForScope(team, team));
    });

    const excludedTeams = ["Cletus Spuckler", "Ralph Wiggum", "Seymour Skinner", "Spotter", "Med Disp Disp"];
    const hasViewAllPermission = hasPermission(user, PERMISSIONS.VIEW_ALL_TEAMS);
    const currentTeam = selectedTeam === 'ALL_TEAMS' ? 'All Teams' : selectedTeam;
    ...
    datasets = datasets.filter(ds => {
        ...
        if (ds.name === 'All Teams') return true;
        return ds.data.some(d => d.retention !== null);
    });
...
appState.profiles.calculateComplianceHistoryData = calculateComplianceHistoryData;
function calculateComplianceHistoryData() {
    ...
    const historicalStubs = appState.loads.historicalStubsData || [];
    const isLiveView = appState.profiles.selectedWeek === 'live';

    // --- FIX: Pre-calculate Driver Dispatcher Map for Live Retention Consistency ---
    const { start: cwStart, end: cwEnd } = getPayrollWeekDateRange(0);
    const allCWLoads = allLoadData
        .filter(d => d.do_date && new Date(d.do_date) >= cwStart && new Date(d.do_date) <= cwEnd && d.status !== 'Canceled')
        .sort((a, b) => new Date(a.do_date) - new Date(b.do_date));

    const currentDriverDispatcherMap = allCWLoads.reduce((acc, load) => {
        if (load.driver && load.dispatcher) acc[load.driver] = load.dispatcher;
        return acc;
    }, {});
    // -------------------------------------------------------------------------------
    ...
    for (let i = 0; i <= weeksToCheck; i++) { 
        ...
        let payDate = new Date(end);
        payDate.setUTCDate(end.getUTCDate() + 3);

        const stdDateStr = payDate.toISOString().split('T')[0];
        const hasStdStubs = historicalStubs.some(s => s.pay_date && new Date(s.pay_date).toISOString().split('T')[0] === stdDateStr);
        
        if (!hasStdStubs) {
             const delayedDate = new Date(end);
             delayedDate.setUTCDate(end.getUTCDate() + 10);
             const delDateStr = delayedDate.toISOString().split('T')[0];
             const hasDelStubs = historicalStubs.some(s => s.pay_date && new Date(s.pay_date).toISOString().split('T')[0] === delDateStr);
             if (hasDelStubs) {
                 payDate = delayedDate;
             }
        }
        ...
        let filteredScoredDispatchers = [];

        // --- LOGIC 1: Live Week (i=0) ---
        if (i === 0 && isLiveView && appState.profiles.allProcessedDispatchers) {
            let source = appState.profiles.allProcessedDispatchers;
            if (contractTypeFilter !== 'all') {
               source = source.filter(d => d.loads && d.loads.some(l => (l.contract_type || 'UNKNOWN').toUpperCase() === contractTypeFilter.toUpperCase()));
            }
             if (selectedCompany !== 'All Companies') {
                source = source.filter(d => d.company === selectedCompany);
             }
             if (selectedFranchise !== 'All Franchises') {
                source = source.filter(d => d.loads.some(l => l.franchise_name === selectedFranchise));
             }
             
             filteredScoredDispatchers = source.map(d => {
                 let wDetails = d.wellnessDetails;
                 if (!wDetails && d.loads) {
                     const passed = d.loads.filter(l => l.wellness_fail === 'GOOD').length;
                     const good = d.loads.filter(l => l.wellness_fail === '-').length;
                     const failed = d.loads.filter(l => l.wellness_fail === 'FAIL').length;
                     wDetails = { passed, good, failed };
                 }
                 return { ...d, wellnessDetails: wDetails || { passed: 0, good: 0, failed: 0 } };
             });

        } 
        // --- LOGIC 1.5: Previous Week (i=1) - Use High Fidelity Data (Live or Cached) ---
        else if (i === 1 && (appState.profiles.selectedWeek === 'live' ? appState.profiles.allProcessedDispatchers_PREV : appState.profiles.cachedWeek1HighFidelity)) {
            let source = (appState.profiles.selectedWeek === 'live') ? appState.profiles.allProcessedDispatchers_PREV : appState.profiles.cachedWeek1HighFidelity;
            if (contractTypeFilter !== 'all') {
               source = source.filter(d => d.loads && d.loads.some(l => (l.contract_type || 'UNKNOWN').toUpperCase() === contractTypeFilter.toUpperCase()));
            }
             if (selectedCompany !== 'All Companies') {
             ...
             filteredScoredDispatchers = source.map(d => {
                 // Calculate wellness details and canceled count if missing
                 let wDetails = d.wellnessDetails;
                 if (!wDetails && d.loads) {
                     const passed = d.loads.filter(l => l.wellness_fail === 'GOOD').length;
                     const good = d.loads.filter(l => l.wellness_fail === '-').length;
                 ...
                 const canceledCount = d.canceled !== undefined ? d.canceled : (d.loads ? d.loads.filter(l => l.status === 'Canceled').length : 0);

                 return { 
                     ...d, 
                     wellnessDetails: wDetails || { passed: 0, good: 0, failed: 0 },
                     canceled: canceledCount
                 };
             });
        }
        // --- LOGIC 2: Historical Weeks (i > 1 or non-Live) ---
        else {
            ...
            if (activeDispatchers.size === 0) {
                weekLoads.forEach(l => { if(l.dispatcher) activeDispatchers.add(l.dispatcher); });
                if (appState.profiles.liveDriverCountData) {
                        appState.profiles.liveDriverCountData.forEach(d => {
                            if(d.dispatcher_name) activeDispatchers.add(d.dispatcher_name);
                        });
                    }
            }
            ...
            const dispatcherMetricsList = activeDispatchersList.map(dispName => {
                ...
                if (i === 0) {
                    ...
                    historicalPool.forEach(driverName => {
                        ...
                        if (statusInfo && statusInfo.contract_status === 'Terminated') {
                            const lastStub = enrichedHistoricalStubs
                                .filter(s => s.driver_name === driverName)
                                .sort((a, b) => new Date(b.pay_date) - new Date(a.pay_date))[0];
                            
                            if (lastStub && lastStub.stub_dispatcher && lastStub.stub_dispatcher !== dispName) {
                                isTransferred = true;
                            } else {
                                isTerminated = true;
                            }
                        ...
                        if (!currentDispatcher) {
                            const liveRecord = appState.profiles.liveDriverCountData.find(d => d.driver_name === driverName);
                            if (liveRecord && liveRecord.dispatcher_name) currentDispatcher = liveRecord.dispatcher_name;
                        }
                        if (currentDispatcher && currentDispatcher !== dispName) isTransferred = true;
                    }

                        if (!isTerminated && !isTransferred) {
                            retainedCount++;
                        }
                    });
                    
                    retentionVal = historicalPool.size > 0 ? (retainedCount / historicalPool.size) * 100 : null;

                } else {
                    const poolStubs = enrichedHistoricalStubs.filter(s => {
                        const pDate = new Date(s.pay_date);
                        return pDate >= fourWeeksAgoStart && pDate <= retentionEnd && s.stub_dispatcher === dispName && (s.retention_status === 'Active' || s.retention_status === 'Terminated' || s.retention_status === 'Start');
                    });
                    const historicalPool = new Set(poolStubs.map(s => s.driver_name));
                    let retainedCount = 0;
                    historicalPool.forEach(driverName => {
                        const driverStubsInWindow = enrichedHistoricalStubs.filter(s => s.driver_name === driverName && new Date(s.pay_date) >= fourWeeksAgoStart && new Date(s.pay_date) <= retentionEnd).sort((a, b) => new Date(b.pay_date) - new Date(a.pay_date));
                        if (driverStubsInWindow.length > 0) {
                            const lastStub = driverStubsInWindow[0];
                ...
                return { name: dispName, loads: dispLoads, team: normalizedTeam, company: company, ooTrucks: ooTrucks, looTrucks: looTrucks, wellness: wellnessScore, wellnessDetails: { passed, good, failed }, goodMoves: goodMovesCount, badMoves: badMovesCount, hiddenMiles: hiddenMiles, lowRpm: lowRpmCount, canceled: canceledCount, overdueLoads: overdueSum, retention4w: retentionVal, medianTenureOO: medianTenureOO, medianTenureLOO: medianTenureLOO, tuesdayOpen: tuesdayOpen, missingPaperwork: missingPaperwork, calculatorActivity: calculatorActivity, calculatorActivityMinutes: calculatorActivityMinutes, trailerDrops: trailerDrops,missingPaperwork: missingPaperwork,
                calculatorActivity: calculatorActivity,
                calculatorActivityCount: daysVisited,
                calculatorActivityMinutes: calculatorActivityMinutes,
                trailerDrops: trailerDrops, trailerRecoveries: trailerRecoveries, rcEntry: rcEntry };
            });
            ...
            if (appState.allHistoricalData) {
                scoredDispatchers.forEach(disp => {
                    const targetDateStr = payDate.toISOString().split('T')[0];
                    const historicalEntry = appState.allHistoricalData.find(d => 
                        d.dispatcherName === disp.name && 
                        d.date.toISOString().split('T')[0] === targetDateStr
                    );
                    if (historicalEntry) {
                        historicalEntry.compliance = disp.complianceScore / 100;
                    }
                });
            }

            filteredScoredDispatchers = scoredDispatchers;

            if (contractTypeFilter !== 'all') {
                filteredScoredDispatchers = filteredScoredDispatchers.filter(d => d.loads && d.loads.some(l => (l.contract_type || 'UNKNOWN').toUpperCase() === contractTypeFilter.toUpperCase()));
            }
            if (selectedCompany !== 'All Companies') {
        ...
        if (groupDispatchers.length === 0) return { weekDate: payDate, dateLabel, badMoves: null, wellness: null, compliance: null, overdue: null, canceled: null, tuesdayOpen: null, calculatorActivity: null, missingPaperwork: null, retention: null };
            ...
            const wellnessScores = groupDispatchers.map(d => d.wellness).filter(w => isFinite(w) && w !== null);
            const medianWellnessScore = wellnessScores.length > 0 ? calculateMedian(wellnessScores) : 0;

            const finalWellness = (i === 1) ? medianWellnessScore : avgWellnessScore;
            ...
            const calcActivities = groupDispatchers.map(d => d.calculatorActivityCount || 0);
            ...
            const medianRcEntry = rcEntriesAll.length > 0 ? Math.round(calculateMedian(rcEntriesAll)) : 0;
        const retentions = groupDispatchers.map(d => d.retention4w).filter(v => v !== null);
        const medianRetention = retentions.length > 0 ? calculateMedian(retentions) : 0;

        return { weekDate: payDate, dateLabel, badMoves: totalBadMoves, canceled: totalCanceled, wellness: finalWellness, compliance: medianCompliance, overdue: medianOverdue, tuesdayOpen: totalTuesdayOpen, calculatorActivity: medianCalculatorActivity, missingPaperwork: totalMissingPaperwork, rcEntry: medianRcEntry, retention: medianRetention, wellnessBreakdown: { passed: totalPassed, good: totalGood, failed: totalFailed } };
    };

        // --- INJECT COMPLIANCE INTO ALL HISTORICAL DATA ---
        if (appState.allHistoricalData) {
            filteredScoredDispatchers.forEach(disp => {
                const targetDate = payDate.toISOString().split('T')[0];
                const historicalEntry = appState.allHistoricalData.find(d => 
                    d.dispatcherName === disp.name && 
        ...
        const weeksAgoSelected = appState.profiles.selectedWeek === 'live' ? 0 : parseInt(appState.profiles.selectedWeek.replace('week_', ''), 10);
        const isSelectedWeek = i === weeksAgoSelected;
       ...
       const allTeamsAgg = aggregateForGroup(filteredScoredDispatchers);
        ...
        const kpiSource = (i === 0 && appState.profiles.cachedLiveKpis) ? appState.profiles.cachedLiveKpis : (isSelectedWeek ? appState.profiles.currentKpis : null);

        if (kpiSource && selectedTeam === 'ALL_TEAMS') {
            if (kpiSource.medianCompliance !== undefined) allTeamsAgg.compliance = kpiSource.medianCompliance;
            if (kpiSource.medianRetentionOptimistic !== undefined) allTeamsAgg.retention = kpiSource.medianRetentionOptimistic;
            if (kpiSource.medianWellness !== undefined) allTeamsAgg.wellness = kpiSource.medianWellness;
            if (kpiSource.canceledLoads !== undefined) allTeamsAgg.canceled = kpiSource.canceledLoads;
            
            // Use currentTeamData ONLY if this iteration matches the user-selected week
            if (isSelectedWeek && appState.profiles.currentTeamData && appState.profiles.currentTeamData.dispatchers) {
                const currentDispatchers = appState.profiles.currentTeamData.dispatchers;
                allTeamsAgg.badMoves = currentDispatchers.reduce((sum, d) => sum + (d.badMoves || 0), 0);
                allTeamsAgg.canceled = currentDispatchers.reduce((sum, d) => sum + (d.canceled || 0), 0);
                allTeamsAgg.tuesdayOpen = currentDispatchers.reduce((sum, d) => sum + (d.tuesdayOpen || 0), 0);
                allTeamsAgg.missingPaperwork = currentDispatchers.reduce((sum, d) => sum + (d.missingPaperwork || 0), 0);
                const overdueValues = currentDispatchers.map(d => d.overdueLoads || 0);
                    allTeamsAgg.overdue = overdueValues.length > 0 ? calculateMedian(overdueValues) : 0;
                    const rcValues = currentDispatchers.map(d => d.rcEntry).filter(v => v !== null);
                    allTeamsAgg.rcEntry = rcValues.length > 0 ? Math.round(calculateMedian(rcValues)) : 0;
                    const calcValues = currentDispatchers.map(d => d.calculatorActivityCount).filter(v => v !== null);
                    allTeamsAgg.calculatorActivity = calcValues.length > 0 ? calculateMedian(calcValues) : 0;

                    let currPassed = 0, currGood = 0, currFailed = 0;
                    currentDispatchers.forEach(d => {
                        if (d.loads) {
                            currPassed += d.loads.filter(l => l.wellness_fail === 'GOOD').length;
                            currGood += d.loads.filter(l => l.wellness_fail === '-').length;
                            currFailed += d.loads.filter(l => l.wellness_fail === 'FAIL').length;
                        }
                    });
                    allTeamsAgg.wellnessBreakdown = { passed: currPassed, good: currGood, failed: currFailed };
                }
        ...
        allUniqueTeams.forEach(teamName => {
            ...
            if (targetViewName === teamName) {
                if (isSelectedWeek && appState.profiles.currentKpis) {
                    teamAgg.compliance = appState.profiles.currentKpis.medianCompliance;
                    teamAgg.retention = appState.profiles.currentKpis.medianRetentionOptimistic;
                    
                    // Use currentTeamData ONLY if this iteration matches the user-selected week
                    if (appState.profiles.currentTeamData && appState.profiles.currentTeamData.dispatchers) {
                        const currentDispatchers = appState.profiles.currentTeamData.dispatchers;
                        teamAgg.badMoves = currentDispatchers.reduce((sum, d) => sum + (d.badMoves || 0), 0);
                        teamAgg.canceled = currentDispatchers.reduce((sum, d) => sum + (d.canceled || 0), 0);
                        teamAgg.tuesdayOpen = currentDispatchers.reduce((sum, d) => sum + (d.tuesdayOpen || 0), 0);
                        teamAgg.missingPaperwork = currentDispatchers.reduce((sum, d) => sum + (d.missingPaperwork || 0), 0);
                        const overdueValues = currentDispatchers.map(d => d.overdueLoads || 0);
                        teamAgg.overdue = overdueValues.length > 0 ? calculateMedian(overdueValues) : 0;
                        const rcValues = currentDispatchers.map(d => d.rcEntry).filter(v => v !== null);
                        teamAgg.rcEntry = rcValues.length > 0 ? Math.round(calculateMedian(rcValues)) : 0;
                        const calcValues = currentDispatchers.map(d => d.calculatorActivityCount).filter(v => v !== null);
                        teamAgg.calculatorActivity = calcValues.length > 0 ? calculateMedian(calcValues) : 0;
                    }
                }
            } else if (targetViewName === 'All Teams' && isSelectedWeek && i === 0 && appState.profiles.currentTeamData && appState.profiles.currentTeamData.dispatchers) {
...
function calculateTrailerDropsHistoryData() {
    ...
    if (appState.profiles.allProcessedDispatchers) {
        ...
        if (contractTypeFilter !== 'all') {
            filtered = filtered.filter(d => d.loads && d.loads.some(l => (l.contract_type || 'UNKNOWN').toUpperCase() === contractTypeFilter.toUpperCase()));
        }
        if (selectedCompany !== 'All Companies') {
        ...
        appState.profiles.liveDriverCountData.forEach(d => {
             ...
             if (contractTypeFilter !== 'all') {
                const raw = String(d.contract_type || '').toUpperCase();
                if (raw !== contractTypeFilter.toUpperCase()) isValid = false;
            }
    ...
    for (let i = 0; i <= weeksToCheck; i++) { 
        ...
        const allTeamsDrops = aggregateForGroup('All Teams');
        const weeksAgoSelected = appState.profiles.selectedWeek === 'live' ? 0 : parseInt(appState.profiles.selectedWeek.replace('week_', ''), 10);
        if (i === weeksAgoSelected && appState.profiles.currentKpis) {
            if (appState.profiles.currentKpis.trailerDrops !== undefined) allTeamsDrops.drops = appState.profiles.currentKpis.trailerDrops;
        }
        datasets["All Teams"].push(allTeamsDrops);
        allUniqueTeams.forEach(teamName => {
...
function calculateGeneralHistoryData() {
    ...
    const getDataForWeek = (weekIndex) => {
        ...
        // Is Live if index is 0
        const isLiveWeek = weekIndex === 0;
        
        let filteredData;
        let isUsingLiveData = false;

        // Try to find historical stubs for this week first (unless it's strictly the live week)
        ...
        const matchingStubs = historicalStubs.filter(s => s.pay_date && s.pay_date.split('T')[0] === targetDateStr);

        // Logic: Use Stubs if available and not live week. Otherwise, calculate from Loads.
        if (matchingStubs.length > 0 && !isLiveWeek) {
            filteredData = matchingStubs;
            isUsingLiveData = false;
        } else {
            // No stubs found (Payroll not run yet) OR it's the live week -> Use Loads
        ...
        return { data: filteredData, isUsingLiveData, end, start };
    };
    ...
    // Iterate Weeks
    for (let i = weeksToCheck; i >= 0; i--) {
        const { data: weekData, isUsingLiveData, end, start } = getDataForWeek(i);
        const payDate = new Date(end);
        payDate.setUTCDate(end.getUTCDate() + 3);
        ...
        const aggregate = (items) => {
            ...
            const totalMiles = isUsingLiveData
                ? activeItems.reduce((sum, l) => sum + (l.trip_miles || 0) + (l.deadhead_miles || 0), 0)
                : activeItems.reduce((sum, s) => sum + (s.total_miles || 0), 0);
            
            const activeTrucks = new Set(activeItems.map(d => isUsingLiveData ? d.driver : d.driver_name)).size;
            ...
            if (!isUsingLiveData) {
                ...
                // If using live data (or loads for past week), we still want Balance from MOST RECENT stubs available
        ...
        allUniqueTeams.forEach(teamName => {
            const teamItems = weekData.filter(d => {
                const t = getGroupedTeamName(isUsingLiveData ? d.team : d.stub_team);
                return t === teamName;
            });
        ...
        if (dispatcherNameFromAccess) {
             const dispItems = weekData.filter(d => {
                 const dName = (isUsingLiveData ? d.dispatcher : d.stub_dispatcher) || '';
...
export function renderGeneralHistoryChart(datasets, metricKey) {
    ...
    datasets = datasets.filter(ds => {
        ...
        if (ds.name === 'All Teams') return true;
        return ds.data.some(d => d[metricKey] !== null);
    });
    ...
    const formatY = (d) => {
        ...
        if (metricKey === 'calculatorActivity') return d;
        if (['badMoves', 'canceled', 'tuesdayOpen', 'missingPaperwork', 'rcEntry'].includes(metricKey)) return d;
        return d;
    };
