...
/**
 * Helper to calculate LIVE retention.
 * Added poolDateOverride to allow forcing a specific date context.
 */
export function calculateLiveRetention(historicalStubs, liveContractStatus, liveDriverCounts, isDriverValidForViewFn, poolDateOverride = null) {
    // 1. Identify the Most Recent Pay Date (Pool Source)
    let targetDateString = poolDateOverride;

    if (!targetDateString) {
        // Fallback: Try to find the latest date in the provided stubs if no override provided
        if (historicalStubs && historicalStubs.length > 0) {
            let maxDate = null;
            ...
            if (maxDate) targetDateString = maxDate.toISOString().split('T')[0];
        }
    }

    // Final Fallback: Use standard calculation (Last Week's Thursday) if still null
    if (!targetDateString) {
        const { end: prevEnd } = getPayrollWeekDateRange(1);
        const prevWeekPayDateTarget = new Date(prevEnd);
        prevWeekPayDateTarget.setUTCDate(prevEnd.getUTCDate() + 3);
        targetDateString = prevWeekPayDateTarget.toISOString().split('T')[0];
    }

    // 2. Build the Pool from Stubs on Target Date
...
export function calculateKpiData(baseData, isLiveData, allDrivers, historicalStubs, contractFilter, currentTeamData, weeksAgo) {
    ...
    // Apply the contract filter dynamically.
    if (contractFilter !== 'all') {
        const contractKey = 'contract_type';
        activeData = activeData.filter(d => {
            const rawContract = String(d[contractKey] || '').trim().toUpperCase();
            return rawContract === contractFilter.toUpperCase();
        });
    }
    ...
    const medianDropRisk = riskScores.length > 0 ? calculateMedian(riskScores) : 0;
   ...
   // --- UPDATED RETENTION LOGIC START ---
    ...
    const filteredStubsForRetention = enrichedStubs.filter(s => {
        ...
        const stubCompany = s.company_name;
        ...
        if (contractFilter !== 'all') {
            ...
            if (raw !== contractFilter.toUpperCase()) return false;
       }
        return true;
    });

    // --- HELPER: Find Best Date for a specific Payroll Week End ---
    const getBestTargetDate = (referenceEndMonday) => {
        const standardDate = new Date(referenceEndMonday);
        standardDate.setUTCDate(referenceEndMonday.getUTCDate() + 3);
        const standardDateString = standardDate.toISOString().split('T')[0];

        const delayedDate = new Date(referenceEndMonday);
        delayedDate.setUTCDate(referenceEndMonday.getUTCDate() + 10);
        const delayedDateString = delayedDate.toISOString().split('T')[0];

        // 1. Check Standard Date
        const hasStandard = filteredStubsForRetention.some(s => s.pay_date && new Date(s.pay_date).toISOString().split('T')[0] === standardDateString);
        if (hasStandard) return standardDateString;
        
        // 2. Check Delayed Date
        const hasDelayed = filteredStubsForRetention.some(s => s.pay_date && new Date(s.pay_date).toISOString().split('T')[0] === delayedDateString);
        if (hasDelayed) return delayedDateString;

        // 3. Fallback to Standard (even if empty, consistent for UI)
    ...
    if (isLiveData) {
        ...
        for (let k = 1; k <= 6; k++) {
            ...
            const pDate = getBestTargetDate(pEnd);
            const hasStubs = filteredStubsForRetention.some(s => s.pay_date && new Date(s.pay_date).toISOString().split('T')[0] === pDate);
            if (hasStubs) {
                targetDateString = pDate;
                break;
            }
        }
        
        if (!targetDateString) {
             const { end: prevWeekEnd } = getPayrollWeekDateRange(1);
             targetDateString = getBestTargetDate(prevWeekEnd);
        }
        ...
        // For HISTORICAL view: Base it on the selected week
        const { end } = getPayrollWeekDateRange(weeksAgo);
        const targetDateString = getBestTargetDate(end);
...
export function getChangeDisplay_Profiles(current, previous, isCurrency = false, isRpm = false, isPercentage = false, lowerIsBetter = false, showSign = true) {
    ...
    let changeText;
    if (isCurrency) {
        changeText = `${sign}$${Math.round(absChange).toLocaleString()}`;
    } else if (isRpm) {
        changeText = `${sign}$${absChange.toFixed(2)}`;
    } else if (isPercentage) {
        // Correctly format percentage changes with one decimal place
        changeText = `${sign}${absChange.toFixed(1)}%`;
    } else {
        changeText = `${sign}${Math.round(absChange).toLocaleString()}`;
    }
...
export function renderProfileHeader(teamData, allTeams, kpis, prevWeekKpis) {
    ...
    const allKpiCards = [
        ...
        { id: 'medianDropRisk', label: 'Median Drop Risk', value: `${kpis.medianDropRisk.toFixed(1)}%`, trend: `<span class="kpi-trend text-gray-500">-</span>` },
        { id: 'balance', label: 'Balance + PO', value: `$${Math.round(kpis.balance).toLocaleString()}`, trend: getChangeDisplay_Profiles(kpis.balance, prevWeekKpis.balance, true, false, false, true, false), isGeneralCard: true },
        { 
            id: 'trailerDrops', 
            label: 'Trailer Drops', 
            value: kpis.trailerDrops.toLocaleString(), 
            trend: getChangeDisplay_Profiles(kpis.trailerDrops, prevWeekKpis.trailerDrops, false, false, false, true, false),
...
export function renderComplianceHistoryChart(datasets, metricKey) {
        ...
        if (visibleDatasets.length > 0 && visibleDatasets[0].data.length > 0) {
            ...
            for (let i = 0; i < numWeeks; i++) {
                const hasData = visibleDatasets.some(ds => {
                    ...
                    return point && point[metricKey] !== null && (point[metricKey] > 0 || i === numWeeks - 1);
                });
                if (hasData) {
    ...
    const formatY = (d) => {
        ...
        if (['badMoves', 'canceled', 'tuesdayOpen', 'calculatorActivity', 'missingPaperwork', 'rcEntry'].includes(metricKey)) return d;
        return d;
    };
    ...
    const updateChart = () => {
        ...
        if (visibleDataPoints.length > 0) {
           ...
           if (['badMoves', 'overdue', 'canceled', 'tuesdayOpen', 'missingPaperwork'].includes(metricKey)) {
                y.domain([0, Math.max(5, Math.max(...vals) + 1)]);
            } else if (metricKey === 'calculatorActivity') {
                y.domain([0, 8]);
            }else {
    ...
    svg.append("rect").attr("width", width).attr("height", height).style("fill", "none").style("pointer-events", "all")
        ...
        .on("mousemove", (event) => {
            ...
            if (d) {
                ...
                metricKey === 'calculatorActivity' ? p.val.toFixed(1) :
                ['badMoves', 'canceled', 'tuesdayOpen', 'missingPaperwork', 'rcEntry'].includes(metricKey) ? p.val.toFixed(1) :
                p.val.toFixed(1)
            );
