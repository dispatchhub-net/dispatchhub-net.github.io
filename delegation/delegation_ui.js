...
import { canViewDispatcher, isAdmin } from '../auth.js';
...
function getDispatcherList() {
    ...
    const fullDispatcherList = combinedDispatcherNames.map((name, index) => {
        ...
        const trucksByContract = myLiveDrivers.reduce((acc, d) => {
            const cType = (d.contract_type || 'UNKNOWN').toUpperCase();
            acc[cType] = (acc[cType] || 0) + 1;
            return acc;
        }, {});
        const ooTrucks = trucksByContract['OO'] || 0; // Legacy fallback
        const looTrucks = trucksByContract['LOO'] || 0; // Legacy fallback
        ...
        const calculateTenure = (contractType) => {
            const activeDriverNames = new Set(prevWeekStubsPool.filter(s => s.stub_dispatcher === name && String(s.contract_type || '').toUpperCase() === contractType.toUpperCase()).map(s => s.driver_name));
        ...
        return {
            ...
            ooTrucks, looTrucks, trucksByContract, currentDrivers: currentDriversList, newStartsCount: uniqueNewDrivers.size, newStartDetails,
...
if (!appState.delegation.capacityMode) {
    ...
    let tableRows = filteredDispatchers.map(d => {
        const contractSpecificTrucks = d.trucksByContract ? (d.trucksByContract[activeContractFilter] || 0) : 0;
        const totalTrucks = Object.values(d.trucksByContract || {}).reduce((a, b) => a + b, 0);
        ...
        <div class="flex flex-wrap justify-between items-center mb-4 p-4 bg-gray-800 rounded-xl border border-gray-700 shadow-lg gap-4">
            <div class="flex items-center gap-6">
               ...
               <div class="relative inline-block text-left w-48 z-30">
                ...
                <div id="delegation-contract-container" class="flex items-center bg-gray-700 rounded-lg p-1 border border-gray-600 transition-all duration-300 flex-wrap">
                    ${(appState.loads?.contractTypes || []).map(c => `
                        <button class="px-3 py-1 text-xs font-bold rounded transition-colors ${activeContractFilter === c.id ? 'bg-teal-500 text-white' : 'text-gray-400 hover:text-white'}" onclick="window.setDelegationContract('${c.id}')">${c.id}</button>
                    `).join('')}
                </div>
            </div>
...
function renderDelegationRow(row, rank) {
    ...
    const activeContractCount = row.trucksByContract ? (row.trucksByContract[activeFilter] || 0) : 0;
    
    let totalText = `${activeFilter}: ${activeContractCount}`;
    if (totalPending > 0 && pendingContract === activeFilter) {
   ...
   const cleanName = row.name.trim().toLowerCase();
    const showProfileIcon = isAdmin() && window.DispatchersUI?.validPerformanceNames?.has(cleanName);

    return `
        <tr class="hover:bg-gray-700 transition-colors group">
            <td class="px-4 py-3 text-center ${rankStyle}">#${rank}</td>
            <td class="px-4 py-3">
                    <div class="font-medium text-white flex items-center gap-2 group">
                        ${row.name}
                        ...
                        ${showProfileIcon ? `
                        <div onclick="event.stopPropagation(); window.openDispatcherProfile('${row.name}')" class="cursor-pointer text-gray-500 hover:text-teal-400 transition-colors opacity-0 group-hover:opacity-100" title="View Profile">
                            ${SILHOUETTE_ICON_SVG}
                        </div>` : ''}
                </div>
...
window.updateAssignment = (dispatcherName, change) => {
    ...
    if (newCount === 0) {
        ...
        // Use our dynamic trucksByContract mapping
        const count = dispatcherData?.trucksByContract ? (dispatcherData.trucksByContract[contract] || 0) : 0;
...
window.openAssignmentModal = (dispatcherName) => {
    ...
    const capConfig = appState.delegation.dispatcherPreferences[dispatcherName] || {};
    const allTypes = appState.loads?.contractTypes || [];

    const allowedOptions = allTypes.filter(type => {
        // If they explicitly unchecked this contract type in settings, hide it. Otherwise allow it.
        return capConfig[type.id.toLowerCase()] !== false; 
    });
...
function renderDispatcherSettingsTab(dispatchers, activeTab) {
    ...
    const hasPerformanceData = window.DispatchersUI?.state?.stats?.[cleanName];
    const profileIcon = (isAdmin() && hasPerformanceData) ? 
        `<div onclick="event.stopPropagation(); window.openDispatcherProfile('${d.name.replace(/'/g, "\\'")}')" class="cursor-pointer text-gray-500 hover:text-teal-400 transition-colors opacity-0 group-hover:opacity-100" title="View Profile">
            ${SILHOUETTE_ICON_SVG}
        </div>` : '';

    return `
        <tr class="border-b border-gray-700 hover:bg-gray-700/50 group">
