...
const splitKeys = ['s_gross', 's_dGross', 's_margin', 's_netPay', 's_consist', 's_loadRate', 's_expense', 's_retention', 's_tenure', 's_rpm', 's_miles', 's_loads', 's_rpmLoad', 's_trucks', 's_weight', 's_netPct', 's_grossPct', 's_marginPct'];
    ...
    loadData: function() {
        ...
        if (appState && appState.allDispatcherNames && appState.allDispatcherNames.length > 0) {
            let rawDispatchers = appState.allDispatcherNames.map((name) => {
                ...
                let team = '';
                let driversByContract = {}; // Dynamic map instead of OO/LOO sets
                ...
                if (appState.loads && appState.loads.historicalStubsData) {
                     appState.loads.historicalStubsData.forEach(s => {
                         ...
                         if (s.stub_dispatcher.trim().toLowerCase() === cleanName) {
                            const type = String(s.contract_type || 'UNKNOWN').toUpperCase();
                            if (!driversByContract[type]) driversByContract[type] = new Set();
                            driversByContract[type].add(s.driver_name);
                        }
                    });
               }
               ...
               const TEAMS_TO_SPLIT = ['Miles', 'Uros', 'Agnius', 'Stefan'];
                ...
                // Safely extract legacy OO/LOO for backwards compatibility, but include dynamic map
                const ooTrucks = driversByContract['OO'] ? driversByContract['OO'].size : 0;
                const looTrucks = Object.keys(driversByContract)
                                    .filter(k => k !== 'OO')
                                    .reduce((sum, k) => sum + driversByContract[k].size, 0);
                ...
                return {
                    ...
                    cardType: cardType,
                    driversByContract: driversByContract, // Pass the dynamic map to the UI
                    ooTrucks: ooTrucks,
                    looTrucks: looTrucks,
                    changeTs: changeTs
                };
    ...
    renderLayout: function() {
        ...
        const allDispatchersGroup = this.state.dispatchers.filter(d => d.ooTrucks + d.looTrucks > 0);
      ...
      container.innerHTML = `
                ...
                <div class="dota-heroes-area">
                    <div class="rows-container">
                        <div class="category-row">
                            ...
                            <div class="cards-grid horizontal-scroll" id="grid-all"></div>
                        </div>
                    </div>
                </div>
        ...
        renderCards(allDispatchersGroup, 'grid-all');
